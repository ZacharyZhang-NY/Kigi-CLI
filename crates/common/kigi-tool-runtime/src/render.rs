...
//! ```rust
//! use serde::Serialize;
//! use kigi_tool_runtime::render::ToolOutput;
...
/// Lets `kigi_tool_types::TaskOutputOutput` be used directly as a `Tool::Output`
/// (handy for stub/test tools and pass-through proxies).
impl ToolOutput for kigi_tool_types::TaskOutputOutput {}

/// Lets `kigi_tool_types::SubagentCompletedOutput` be used directly as a
/// `Tool::Output` (the `task` tool's structured completion output).
impl ToolOutput for kigi_tool_types::SubagentCompletedOutput {}

/// Lets `kigi_tool_types::KillTaskOutput` be used directly as a `Tool::Output`
/// (the `kill_task` tool's typed result / not-found output).
impl ToolOutput for kigi_tool_types::KillTaskOutput {}
