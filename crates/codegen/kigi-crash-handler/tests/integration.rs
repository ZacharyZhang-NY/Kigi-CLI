//! Integration tests for kigi-crash-handler.
//!
...
fn subprocess_entry() {
    ...
    let config = kigi_crash_handler::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    kigi_crash_handler::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob =
        kigi_crash_handler::format::CrashBlob::parse(&data).expect("crash blob should parse");
    ...
    let report = kigi_crash_handler::check_previous_crash(tmp.path())
