...
/// Lenient boolean deserializers (shared via `kigi-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use kigi_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
