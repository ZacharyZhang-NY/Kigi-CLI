...
//!
//! `MemoryBackend` is defined in `kigi-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `kigi-tools`.
/// Mirrors `kigi_shell::session::memory_log::TARGET`.
