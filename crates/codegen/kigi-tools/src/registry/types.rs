...
/// this crate: instead of `kigi-tools` referencing an out-of-tree tool
/// pack, the pack calls
...
impl ToolConfig {
    ...
    /// `kigi_tool_runtime::Tool::id()`. Use this for built-in tools known
    ...
    where
        T: crate::types::tool_metadata::ToolMetadata + kigi_tool_runtime::Tool + Default,
    {
        Self::from(&T::default())
    }
...
impl<T: crate::types::tool_metadata::ToolMetadata + kigi_tool_runtime::Tool> From<&T>
    for ToolConfig
{
    fn from(tool: &T) -> Self {
        Self {
...
pub async fn drain_value_stream(
    mut stream: kigi_tool_runtime::ToolStream<kigi_tool_runtime::TypedToolOutput>,
) -> Result<serde_json::Value, kigi_tool_runtime::ToolError> {
    use futures::StreamExt;
    while let Some(item) = stream.next().await {
        match item {
            kigi_tool_runtime::ToolStreamItem::Progress(_) => continue,
            kigi_tool_runtime::ToolStreamItem::Terminal(result) => {
...
fn stream_no_terminal_error() -> kigi_tool_runtime::ToolError {
...
pub struct FinalizedToolset {
    ...
    workspace_viewer_ctx: Option<kigi_tool_runtime::WorkspaceViewerContext>,
}
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
impl ToolRegistryBuilder {
            ...
            + 'static,
    {
        ...
        let id = kigi_tool_runtime::Tool::id(&tool).as_str().to_string();
        let kind = tool.kind();
...
///
/// Implements the canonical `kigi_tool_runtime::ToolDispatch` trait so the
...
#[async_trait::async_trait]
impl kigi_tool_runtime::ToolDispatch for InnerDispatchForToolset {
    async fn call(
        &self,
        ...
        let result = self
            ...
            .and_then(|output| {
                let value = serde_json::to_value(&output).map_err(|e| {
                    kigi_tool_runtime::ToolError::custom("output_encoding", e.to_string())
                })?;
                Ok(kigi_tool_runtime::TypedToolOutput::from_value(
                    tool_id.clone(),
                    value,
                ))
            });
        kigi_tool_runtime::terminal_only(result)
    }
}
impl FinalizedToolset {
    ...
    fn tool_not_found_error(tool_name: &str) -> kigi_tool_runtime::ToolError {
        let tid = kigi_tool_protocol::ToolId::new(tool_name)
            .unwrap_or_else(|_| kigi_tool_protocol::ToolId::new("unknown").expect("valid"));
        kigi_tool_runtime::ToolError::not_found(tid, format!("Tool not found: {tool_name}"))
    }
    ...
    async fn call_raw(
        ...
        parent_ctx: kigi_tool_runtime::ToolCallContext,
    ) -> Result<crate::types::output::ToolOutput, kigi_tool_runtime::ToolError> {
        ...
        let mut ctx = kigi_tool_runtime::ToolCallContext::new(parent_ctx.call_id.clone());
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = parent_ctx.extensions.get::<kigi_tool_runtime::Cwd>() {
            ctx.extensions.insert((*cwd).clone());
        }
        let tool_id = kigi_tool_protocol::ToolId::new(&registry_id)
            .unwrap_or_else(|_| kigi_tool_protocol::ToolId::new("unknown").expect("valid"));
    ...
    pub async fn call(
        ...
        while let Some(item) = stream.next().await {
            match item {
                kigi_tool_runtime::ToolStreamItem::Progress(_) => continue,
                kigi_tool_runtime::ToolStreamItem::Terminal(result) => return result,
            }
        }
        Err(stream_no_terminal_error())
    }
    ...
    ///
    /// [`ToolStream`]: kigi_tool_runtime::ToolStream
    /// [`ToolStreamItem::Progress`]: kigi_tool_runtime::ToolStreamItem::Progress
    pub fn call_streaming(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> kigi_tool_runtime::ToolStream<ToolRunResult> {
        use futures::StreamExt;
        ...
        Box::pin(async_stream::stream! {
            ...
            kigi_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; } }; let
            ...
            match item { kigi_tool_runtime::ToolStreamItem::Progress(p) => { yield
            kigi_tool_runtime::ToolStreamItem::Progress(p); }
            kigi_tool_runtime::ToolStreamItem::Terminal(Err(e)) => { yield
            kigi_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; }
            kigi_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => { let run_result
            = this.finalize_output(typed.value, & output_converter,
            effective_tool_name). await; yield
            kigi_tool_runtime::ToolStreamItem::Terminal(run_result); return; } } }
            yield
            kigi_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
    ...
    fn prepare_dispatch(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> Result<DispatchParts, kigi_tool_runtime::ToolError> {
        let (registry_id, output_converter, reverse_params) = {
            let tools = self.tools.read();
            let entry = tools
                .iter()
        ...
        let rt_call_id = kigi_tool_protocol::ToolCallId::new(tool_call_id)
            .unwrap_or_else(|_| kigi_tool_protocol::ToolCallId::new_v7());
        let mut ctx = kigi_tool_runtime::ToolCallContext::new(rt_call_id);
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = cwd_override {
            ctx.extensions.insert(kigi_tool_runtime::Cwd(cwd));
        }
        if let Some(ref version) = contract_version {
            ctx.extensions
                .insert(kigi_tool_runtime::BehaviorVersion(version.clone()));
        }
    ...
    ///
    /// The tool must implement `kigi_tool_runtime::Tool + ToolMetadata`.
    ...
    where
        T: kigi_tool_runtime::Tool + ToolMetadata + std::fmt::Debug + Send + Sync + 'static,
        T::Output: serde::Serialize,
    {
        ...
        let kind = tool.kind();
        let registry_id = kigi_tool_runtime::Tool::id(&tool).as_str().to_owned();
    ...
    pub fn unregister_tools_by_prefix(&self, prefix: &str) -> usize {
        ...
        let to_remove: Vec<_> = tools
            ...
            .filter_map(|t| kigi_tool_protocol::ToolId::new(&t.registry_id).ok())
            .collect();
    ...
    pub fn unregister_tool_by_name(&self, name: &str) -> bool {
        ...
        let tool_id = tools
            ...
            .and_then(|t| kigi_tool_protocol::ToolId::new(&t.registry_id).ok());
        let before = tools.len();
...
mod tests {
    ...
    impl kigi_tool_runtime::Tool for FakeMcpTool {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> kigi_tool_protocol::ToolId {
            kigi_tool_protocol::ToolId::new("fake_mcp").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::kigi_tool_runtime::ListToolsContext,
        ) -> kigi_tool_types::ToolDescription {
            kigi_tool_types::ToolDescription::new("fake_mcp", &self.description)
        }
        async fn run(
            &self,
    ...
    impl kigi_tool_runtime::Tool for NonStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> kigi_tool_protocol::ToolId {
            kigi_tool_protocol::ToolId::new("non_streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::kigi_tool_runtime::ListToolsContext,
        ) -> kigi_tool_types::ToolDescription {
            kigi_tool_types::ToolDescription::new("non_streaming_stub", "non-streaming stub")
        }
        async fn run(
            &self,
            _ctx: kigi_tool_runtime::ToolCallContext,
    ...
    impl kigi_tool_runtime::Tool for StreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> kigi_tool_protocol::ToolId {
            kigi_tool_protocol::ToolId::new("streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::kigi_tool_runtime::ListToolsContext,
        ) -> kigi_tool_types::ToolDescription {
            kigi_tool_types::ToolDescription::new("streaming_stub", "streaming stub")
        }
        async fn run(
            &self,
            _ctx: kigi_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            _ctx: kigi_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> kigi_tool_runtime::ToolStream<String> {
            Box::pin(futures::stream::iter(vec![
                kigi_tool_runtime::ToolStreamItem::Progress(
                    kigi_tool_runtime::ToolProgress::Text {
                        text: "progress-1".into(),
                    },
                ),
                kigi_tool_runtime::ToolStreamItem::Terminal(Ok("terminal-value".to_string())),
            ]))
        }
    }
        ...
        while let Some(item) = stream.next().await {
            match item {
                kigi_tool_runtime::ToolStreamItem::Progress(p) => {
                    ...
                    assert!(matches!(p, kigi_tool_runtime::ToolProgress::Text { .. }));
                    progress_count += 1;
                }
                kigi_tool_runtime::ToolStreamItem::Terminal(result) => {
    ...
    impl kigi_tool_runtime::Tool for NoTerminalStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> kigi_tool_protocol::ToolId {
            kigi_tool_protocol::ToolId::new("no_terminal_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::kigi_tool_runtime::ListToolsContext,
        ) -> kigi_tool_types::ToolDescription {
            kigi_tool_types::ToolDescription::new("no_terminal_stub", "no-terminal stub")
        }
        async fn run(
            &self,
            _ctx: kigi_tool_runtime::ToolCallContext,
    ...
    async fn prepare_dispatch_stamps_workspace_viewer_ctx_when_present() {
        let (toolset, _tmp) =
            toolset_with_viewer_ctx(Some(kigi_tool_runtime::WorkspaceViewerContext {
    ...
    async fn bash_streaming_progress_emitted_via_finalized_toolset_when_gate_on() {
        ...
        while let Some(item) = stream.next().await {
            match item {
                kigi_tool_runtime::ToolStreamItem::Progress(_) => progress_count += 1,
                kigi_tool_runtime::ToolStreamItem::Terminal(r) => {
