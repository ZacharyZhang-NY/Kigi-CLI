...
pub fn shell_env_overrides() -> HashMap<String, String> {
    HashMap::from([
        ...
        (
            ...
            crate::util::KIGI_AGENT_ENV_VALUE.to_string(),
        ),
    ])
}
...
dump_bash_state() {
  ...
  local env_vars
  env_vars=$(builtin export -p 2>/dev/null | command grep -viE '_proxy=|KIGI_SANDBOX|KIGI_AGENT=|SUDO_ASKPASS|KIGI_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
...
function dump_zsh_state() {
  ...
  local env_vars
  env_vars=$(builtin typeset -xp 2>/dev/null | command grep -viE '_proxy=|KIGI_SANDBOX|KIGI_AGENT=|SUDO_ASKPASS|KIGI_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
  _emit_encoded "$env_vars" "ENV_VARS_B64"
...
impl ShellKind {
    ...
    pub fn detect() -> Self {
        match kigi_config::shell::detect_unix_shell_kind() {
            kigi_config::shell::UnixShellKind::Bash => Self::Bash,
            kigi_config::shell::UnixShellKind::Zsh => Self::Zsh,
        }
    }
    ...
    pub fn binary_path(&self) -> &'static str {
        let kind = match self {
            ...
            Self::Zsh => kigi_config::shell::UnixShellKind::Zsh,
        };
        kigi_config::shell::unix_shell_path(kind)
    }
...
mod tests {
    ...
    fn shell_env_overrides_marks_agent_terminal() {
        ...
        assert_eq!(
            env.get(crate::util::KIGI_AGENT_ENV).map(String::as_str),
            Some(crate::util::KIGI_AGENT_ENV_VALUE)
        );
