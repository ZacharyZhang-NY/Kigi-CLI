...
impl kigi_tool_runtime::Tool for KillTaskTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> kigi_tool_protocol::ToolId {
        kigi_tool_protocol::ToolId::new("kill_task").expect("valid tool id")
    }
...
mod tests {
    ...
    fn make_ctx_with_version(
        ...
        ctx.extensions
            .insert(kigi_tool_runtime::BehaviorVersion(version.to_owned()));
        ctx
    }
