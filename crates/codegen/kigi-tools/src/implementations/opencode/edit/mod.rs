...
impl kigi_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> kigi_tool_protocol::ToolId {
        kigi_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
        ...
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            kigi_tool_runtime::ToolError::execution(
                kigi_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(kigi_tool_runtime::Tool::id(&tool).as_str(), "edit");
