...
impl kigi_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> kigi_tool_protocol::ToolId {
        kigi_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(kigi_tool_runtime::ToolError::execution(
                    kigi_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = kigi_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        ...
        let tool = BashTool;
        assert_eq!(kigi_tool_runtime::Tool::id(&tool).as_str(), "bash");
