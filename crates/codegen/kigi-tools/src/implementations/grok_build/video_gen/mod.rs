...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> kigi_file_utils::s3::S3StaticCredentials {
        kigi_file_utils::s3::S3StaticCredentials {
...
impl VideoGenClient {
    ...
    ) -> Result<VideoOutcome, kigi_tool_runtime::ToolError> {
    ...
    async fn download_video(&self, url: &str) -> Result<Vec<u8>, kigi_tool_runtime::ToolError> {
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, kigi_tool_runtime::ToolError> {
        ...
        let upload_url = kigi_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_zdr_get_url(
        ...
        let url = kigi_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
async fn resolve_image_reference(value: &str) -> Result<String, kigi_tool_runtime::ToolError> {
    ...
    let (_w, _h, mime) =
        crate::util::image_validate::validate_image_bytes(&raw_bytes).map_err(|e| {
            kigi_tool_runtime::ToolError::invalid_arguments(format!("invalid image reference: {e}"))
        })?;
...
fn validate_imagine_duration(duration: Option<u32>) -> Result<(), kigi_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), kigi_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, kigi_tool_runtime::ToolError> {
...
) -> Result<MediaGenOutput, kigi_tool_runtime::ToolError> {
    match outcome {
...
impl kigi_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> kigi_tool_protocol::ToolId {
        kigi_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
impl kigi_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> kigi_tool_protocol::ToolId {
        kigi_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    async fn image_to_video_rejects_bad_duration() {
        ...
        let err = kigi_tool_runtime::Tool::run(
            &tool,
