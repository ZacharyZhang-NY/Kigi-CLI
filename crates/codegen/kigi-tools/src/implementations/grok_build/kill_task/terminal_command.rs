...
impl kigi_tool_runtime::Tool for KillTerminalCommandTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> kigi_tool_protocol::ToolId {
        kigi_tool_protocol::ToolId::new("kill_terminal_command").expect("valid tool id")
    }
    ...
    ) -> Result<KillTaskOutput, kigi_tool_runtime::ToolError> {
        kigi_tool_runtime::Tool::run(&KillTaskTool, ctx, input).await
    }
}
