    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        Expr::Value(ToolRequirement::Tool {
            namespace: crate::types::tool_metadata::ToolMetadata::tool_namespace(&ExitPlanModeTool)
                .to_string(),
            id: kigi_tool_runtime::Tool::id(&ExitPlanModeTool).to_string(),
            if_params: None,
        })
    }
}

impl kigi_tool_runtime::Tool for EnterPlanModeTool {
    type Args = EnterPlanModeInput;
    type Output = EnterPlanModeOutput;

    fn id(&self) -> kigi_tool_protocol::ToolId {
        kigi_tool_protocol::ToolId::new("enter_plan_mode").expect("valid tool id")
    }
    ...
    ) -> Result<EnterPlanModeOutput, kigi_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn no_absolute_path_does_not_write() {
        ...
        assert_eq!(plan_file_path, ".kigi/plan.md");
    ...
    async fn uses_plan_file_path_resource_when_set() {
        let mut resources = Resources::new();
        let session_plan = PathBuf::from("/home/user/.kigi/sessions/abc123/plan.md");
        ...
        assert!(!plan_file_path.contains(".kigi/plan.md"));
    }
    ...
    async fn falls_back_to_cwd_when_no_plan_file_path_resource() {
        ...
        assert_eq!(plan_file_path, "/workspace/my-project/.kigi/plan.md");
    }
    ...
    async fn plan_file_path_prefers_resource_over_cwd() {
        ...
        resources.insert(PlanFilePath(PathBuf::from(
            "/home/user/.kigi/sessions/xyz/plan.md",
        )));
        let shared = resources.into_shared();
        ...
        assert_eq!(plan_file_path, "/home/user/.kigi/sessions/xyz/plan.md");
        assert!(!plan_file_path.contains(".kigi/plan.md"));
    }
}
