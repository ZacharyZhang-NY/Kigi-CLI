...
use kigi_config::shell::AmpersandSemantics;
use regex::Regex;
...
static BASH_CAPABILITIES: LazyLock<kigi_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| kigi_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(kigi_tool_protocol::ToolScope::Write),
        streaming: Some(kigi_tool_protocol::StreamingSpec {
...
impl kigi_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<kigi_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => kigi_tool_runtime::ToolOutput::chat_completion_output(bash),
...
/// [`kigi_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`KIGI_FOREGROUND_BLOCK_BUDGET_MS`
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = kigi_config::shell::chain_separator();
...
impl kigi_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> kigi_tool_protocol::ToolId {
        kigi_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: kigi_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> kigi_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield kigi_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield kigi_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield kigi_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, kigi_tool_runtime::ToolError> {
        ...
        let ampersand = kigi_config::shell::ampersand_semantics();
...
mod tests {
    ...
    fn read_chunk_progress(p: &kigi_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            kigi_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let kigi_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = kigi_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, kigi_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                kigi_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = kigi_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(kigi_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod versioned_background_check {
        ...
        fn legacy_rt_ctx(
            ...
            let mut ctx =
                kigi_tool_runtime::ToolCallContext::new(kigi_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(kigi_tool_runtime::BehaviorVersion(
