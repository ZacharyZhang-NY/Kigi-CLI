...
use kigi_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = kigi_sampling_types::reported_cost_ticks(
        value
...
impl SamplingClient {
    ...
    pub async fn conversation_stream_responses(
        ...
        let extra_tools = kigi_sampling_types::extra_raw_tools(&request.hosted_tools);
...
mod tests {
    ...
    use kigi_sampling_types::types::ChatRequestMessage;
    ...
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = kigi_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
