...
use anyhow::Result;
use kigi_shell::agent::app::{run_leader, run_stdio_agent};
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    use kigi_log::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(kigi_log::sampling_log::layer())
        .with(kigi_log::instrumentation::layer())
...
async fn run_setup_command(json: bool) {
    ...
    if !managed_config::has_principal() {
        ...
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export KIGI_DEPLOYMENT_KEY=<your-key>");
        } else {
...
) -> Result<(LeaderDescriptor, kigi_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = kigi_shell::leader::resolve_leader_target(target)
        .await
...
fn leader_info_json(
    ...
    info: Option<&kigi_shell::leader::ControlPayload>,
...
async fn run_agent_command(
    agent_args: Box<kigi_tui::app::AgentArgs>,
    ...
    kigi_log::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => kigi_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(kigi_shell::sampling::types::parse_canonical_effort_token);
    ...
    match agent_args.mode {
        ...
        AgentCmd::Serve(a) => {
            ...
            let server_config = kigi_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            kigi_shell::agent::run_agent_server(server_config, agent_config).await
        }
        AgentCmd::Leader(a) => {
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(kigi_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = kigi_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = kigi_shell::managed_config::sync().await {
                                            tracing::warn!(
...
/// (`[dashboard].enabled = false` / `KIGI_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !kigi_tui::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.kigi/config.toml and \
             unsetting KIGI_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("KIGI_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<kigi_tui::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(kigi_tui::memory_trace::AllocatorStats {
...
fn main() {
    kigi_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    kigi_tui::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        kigi_tui::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        kigi_tui::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    if let Some(code) = kigi_tui::app::mermaid_worker::maybe_run_render_subprocess() {
    ...
    kigi_tui::docs::extract_user_guide_docs(&kigi_shell::util::kigi_home::kigi_home());
    kigi_crash_handler::install_terminal_restore_only();
    if kigi_shell::util::config::load_crash_handler_enabled_sync() {
        ...
        if !kigi_crash_handler::install(kigi_crash_handler::CrashHandlerConfig {
    ...
    let crashed = kigi_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("KIGI_COMPACTION_MODE", mode) };
    }
    ...
    if let Some(ref path) = args.debug_file {
        unsafe {
            ...
            std::env::remove_var("KIGI_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("KIGI_DEBUG_LOG", "1");
        set_if_unset("KIGI_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        kigi_tui::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return kigi_tui::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        kigi_tui::app::cli::SandboxStartup::Apply(profile) => profile,
        kigi_tui::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    kigi_shell::config::apply_sandbox(None, sandbox_profile_arg.as_deref(), args.cwd.as_deref());
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        kigi_update::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return kigi_tui::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                return kigi_tui::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return kigi_tui::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return kigi_tui::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return kigi_tui::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return kigi_tui::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Logout => {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                kigi_shell::auth::run_cli_logout(&config)?;
                kigi_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = kigi_tui::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(kigi_tui::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == kigi_tui::headless::OutputFormat::Plain {
                args.output_format = kigi_tui::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return kigi_tui::headless::run_single_turn(
            prompt,
            args.verbatim,
            kigi_tui::headless::HeadlessOptions {
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    if std::env::var_os("KIGI_DISABLE_AUTOUPDATER").is_some() {
        return false;
    }
    true
}
...
/// `KIGI_DISABLE_AUTOUPDATER`, debug builds) is layered on separately via
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in kigi_shell::leader::discover_leaders().await {
        if d.classification != kigi_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        match client
            ...
            .await
        {
            Ok(Ok(kigi_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
...
mod tests {
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("KIGI_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
    ...
    /// `KIGI_AGENT_DASHBOARD=0` disables the feature — the subcommand
