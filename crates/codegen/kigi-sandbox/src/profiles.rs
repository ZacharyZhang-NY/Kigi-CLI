...
//! `off`. Custom profiles via `~/.kigi/sandbox.toml` or `.kigi/sandbox.toml`.
...
/// Load sandbox config from `~/.kigi/sandbox.toml` and `.kigi/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.kigi/sandbox.toml
    let global_path = kigi_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.kigi/sandbox.toml (additive only)
    let project_path = workspace.join(".kigi").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&kigi_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".kigi").join("sandbox.toml")).unwrap_or_default();
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.kigi/sandbox.toml or .kigi/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
