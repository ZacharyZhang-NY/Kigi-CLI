...
use kigi_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (kigi_tui::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    kigi_tui::glyphs::ballot_x(),
...
mod tests {
    use super::*;
    use kigi_shell::tools::{TodoItem, TodoPriority};
