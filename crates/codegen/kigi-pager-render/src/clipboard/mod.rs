...
//! `kigi-ratatui-textarea`, and adds [`SystemClipboard`] backed by `arboard`.
//!
//! Multi-fire writes (native / tmux / OSC 52); user-facing success is [`trust`].

mod trust;

pub use kigi_ratatui_textarea::{ClipboardProvider, InternalClipboard};
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(kigi_shared::clipboard::is_containerized_without_display)
}
...
pub fn osc52_sink_active() -> bool {
    ...
    *SINK.get_or_init(|| {
        std::env::var_os("KIGI_OSC52_SINK").is_some()
            || std::env::var_os("LC_KIGI_OSC52_SINK").is_some()
    })
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if kigi_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = kigi_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = kigi_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`kigi_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        kigi_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = kigi_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match kigi_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    kigi_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !kigi_shared::clipboard::x11_display_env_present() {
        return None;
    }
    kigi_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && kigi_shared::clipboard::x11_display_env_present()
    }
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    match kigi_shared::clipboard::get_attachments() {
...
pub use kigi_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    kigi_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    kigi_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    kigi_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(kigi_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    match kigi_shared::clipboard::get_image() {
        Ok(img) => Ok(img),
        Err(e) => {
