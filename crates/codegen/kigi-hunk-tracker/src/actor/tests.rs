...
impl TestHarness {
    ...
    fn populate(&self, files: usize, files_per_dir: usize) {
        kigi_test_utils::git::write_fanout_tree(&self.working_dir, files, files_per_dir);
    ...
    fn feature_branch(&self, picks: usize) -> String {
        kigi_test_utils::git::make_feature_branch(&self.working_dir, picks)
    }
}
...
/// never wall-clock.
///
/// Knobs: KIGI_PERF_GIT_FILES (default 5000), KIGI_PERF_GIT_PICKS (default 10).
#[tokio::test]
#[ignore = "perf repro; run with --ignored --nocapture"]
async fn refresh_storm_scan_count_during_rebase() {
    let files = env_usize("KIGI_PERF_GIT_FILES", 5000);
    let picks = env_usize("KIGI_PERF_GIT_PICKS", 10);
    ...
    let t = Instant::now();
    kigi_test_utils::git::run_git_with_env(
        &wd,
        &["rebase", "-i", &base],
