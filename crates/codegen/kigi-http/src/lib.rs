...
//! `kigi_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `KIGI_POOL_*` /
//! `KIGI_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `KIGI_SAMPLER_SHARED_CLIENT=0` falls back to
...
// functions below.
pub use kigi_sampler::OriginClientInfo;

/// Construct an [`OriginClientInfo`] from `KIGI_CLIENT_NAME` /
/// `KIGI_CLIENT_VERSION` env vars. Returns `None` when
/// `KIGI_CLIENT_NAME` is unset.
pub fn origin_client_info_from_env() -> Option<OriginClientInfo> {
    std::env::var("KIGI_CLIENT_NAME")
        .ok()
        .map(|product| OriginClientInfo {
            product,
            version: std::env::var("KIGI_CLIENT_VERSION").ok(),
        })
}
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn kigi_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(kigi_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
