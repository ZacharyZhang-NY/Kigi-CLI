...
//! OS-protected layers (root-owned `/etc/kigi`, MDM).

use std::path::Path;

use crate::paths::user_kigi_home;
...
pub fn is_managed_config_stale_for(identity: &ServingIdentity) -> bool {
    managed_config_stale_at(user_kigi_home().as_deref(), identity)
}
...
pub fn mark_managed_config_synced(marker: SyncMarker<'_>) {
    if let Some(home) = user_kigi_home() {
...
pub fn managed_deployment_id(key_fingerprint: &str) -> Option<String> {
    managed_deployment_id_at(user_kigi_home()?.as_path(), key_fingerprint)
}
...
pub fn managed_config_identity_changed(
    ...
    user_kigi_home().is_some_and(|home| {
...
pub fn managed_policy_compromised_for(identity: &ServingIdentity) -> bool {
    user_kigi_home().is_some_and(|home| managed_policy_compromised_for_at(&home, identity))
}
...
/// Override with `KIGI_DEPLOYMENT_CONFIG_CACHE_TTL_SECS` for testing.
fn managed_config_stale_threshold() -> std::time::Duration {
    if let Ok(s) = std::env::var("KIGI_DEPLOYMENT_CONFIG_CACHE_TTL_SECS")
