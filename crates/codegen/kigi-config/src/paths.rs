//! Filesystem locations for Kigi config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static KIGI_SHARE_DIR: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user Kigi directory (`~/.kigi`, canonicalized) used when
/// `KIGI_SHARE_DIR` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`kigi_home()`] is the default without duplicating the computation.
///
...
/// `~/.kigi/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `kigi_fast_worktree::db::resolve_kigi_home` (deliberately standalone crate).
pub fn default_kigi_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".kigi")
}

/// Per-user config directory: `$KIGI_SHARE_DIR` or `~/.kigi`. Created if needed.
pub fn kigi_home() -> PathBuf {
    KIGI_SHARE_DIR
        .get_or_init(|| {
            let kigi_home = if let Ok(v) = std::env::var("KIGI_SHARE_DIR") {
                PathBuf::from(v)
            } else {
                default_kigi_home()
            };
...
/// The user-global Kigi home, but only when one genuinely resolves: `Some` when
/// `$KIGI_SHARE_DIR` is set or a home directory is found, `None` otherwise. Unlike
/// [`kigi_home()`], this never falls back to a cwd-relative `.kigi`, so callers
/// that *scan* user-global Kigi resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.kigi` tree for the user-global one when no home resolves.
pub fn user_kigi_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("KIGI_SHARE_DIR").is_some() || std::env::home_dir().is_some();
    resolvable.then(kigi_home)
}

/// Canonical kigi application path: `$KIGI_SHARE_DIR/bin/kigi` (Unix) or `kigi.exe` (Windows).
pub fn kigi_application() -> PathBuf {
    let name = if cfg!(windows) { "kigi.exe" } else { "kigi" };
    kigi_home().join("bin").join(name)
}

/// System-wide config directory: `/etc/kigi/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/kigi"))
    } else {
        None
    }
}
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    kigi_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = kigi_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_kigi_home_has_no_verbatim_prefix() {
        ...
        let home = default_kigi_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".kigi"));
    }
