...
/// with `KIGI_MANAGED_CONFIG_URL`; only applies to `requirements.toml`.
pub(crate) const FAIL_CLOSED_ENV: &str = "KIGI_MANAGED_CONFIG_FAIL_CLOSED";
...
impl RequirementsSource {
    ...
    /// id (`ai.x.kigi:…`). For diagnostics and matching only; the MDM layer has no
...
pub struct RequirementsLayer {
    ...
    /// not re-derive from the source (`KIGI_SHARE_DIR`-influenced, could carry `..`).
    pub is_system: bool,
}
...
pub fn requirements_layers() -> Vec<RequirementsLayer> {
    let mut out = Vec::new();
    if let Some(user_path) = user_kigi_home().map(|g| g.join("requirements.toml"))
...
pub(crate) fn load_requirements() -> Option<toml::Value> {
    load_user_requirements(user_kigi_home().as_deref())
}

/// User requirements layer from `<home>/requirements.toml`, or `None` with no
/// resolvable user home (rather than reading a cwd-relative `.kigi`).
...
fn validate_requirements_value(
    ...
    let Ok(version) = kigi_version::installed_semver() else {
        return Ok(());
    };
...
pub fn validate_requirements() -> Result<(), RequirementsError> {
    validate_user_requirements(user_kigi_home().as_deref())?;
    if let Some(dir) = system_config_dir() {
...
/// no-op (no cwd-relative `.kigi/requirements.toml` is read or enforced).
