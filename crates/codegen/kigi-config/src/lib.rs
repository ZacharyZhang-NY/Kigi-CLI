...
//! 2. `$KIGI_SHARE_DIR/managed_config.toml`
//! 3. `$KIGI_SHARE_DIR/config.toml`
//! 4. `$KIGI_SHARE_DIR/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/kigi/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.kigi`, admin-forced) — macOS only
//!
...
pub use paths::{
    ...
    default_kigi_home, encode_cwd_dirname, ensure_sessions_cwd_dir, kigi_application, kigi_home,
    sessions_cwd_dir, system_config_dir, user_kigi_home,
};
pub use validation::{
