...
//!
//! Searches `.kigi/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.kigi/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.kigi/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".kigi/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.kigi/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.kigi/agents` / `.claude/agents`) under each
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, kigi_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.kigi/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.kigi/agents/` walking from `cwd` up to repo root
/// 2. `~/.kigi/agents/` (user-level)
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, kigi_home) {
        if dir.is_dir() {
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, kigi_home);
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, kigi_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, kigi_home) {
        return Some(def);
    }
...
/// Load project agent definitions from every `.kigi/agents` / `.claude/agents`
...
mod tests {
    ...
    fn user_agent_dirs_dedups_legacy_when_kigi_home_is_dot_grok() {
        ...
        assert_eq!(
            count, 1,
            "no duplicate ~/.kigi/agents when kigi_home == ~/.kigi"
        );
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".kigi").join("agents");
        let agents_dir_2 = inner_dir.join(".kigi").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".kigi").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".kigi")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".kigi")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".kigi").join("agents");
        let bundled_dir = home.join(".kigi").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".kigi")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".kigi")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".kigi").join("agents");
        let bundled_dir = home.join(".kigi").join("bundled").join("agents");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.kigi/agents/) agent named "explore" should NOT shadow
