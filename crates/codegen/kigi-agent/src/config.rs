//! Agent definition types — parsed from `.kigi/agents/*.md` files.
...
use kigi_tools::implementations::use_tool;
...
/// Portable agent identity — parsed from .kigi/agents/*.md.
...
pub struct AgentDefinition {
    ...
    pub capability_mode: Option<kigi_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .kigi/agents/ (project-level, highest priority)
    Project,
    /// ~/.kigi/agents/ (user-level)
    User,
...
pub enum MemoryScope {
    /// `~/.kigi/agent-memory/<name>/`
    User,
    /// `<project>/.kigi/agent-memory/<name>/`
    Project,
    /// `<project>/.kigi/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: kigi_config::kigi_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".kigi/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".kigi/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `kigi_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        if path_str.contains(".kigi/agents/") || path_str.contains(".kigi\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".kigi/bundled/agents/")
            || path_str.contains(".kigi\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: kigi_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: kigi_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: kigi_tool_types::PLAN_SUBAGENT.description.to_string(),
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.kigi/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.kigi/agent-memory-local/a")
        );
