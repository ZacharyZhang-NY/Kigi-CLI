...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] kigi_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `kigi-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `kigi_tools::ToolError`) happens
