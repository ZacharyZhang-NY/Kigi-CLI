...
//! `kigi-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.kigi/skills
    Local,
    /// repo_root/.kigi/skills
    Repo,
    /// ~/.kigi/skills
    User,
    /// ~/.kigi/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.kigi/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the kigi-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.kigi/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.kigi/skills"},
