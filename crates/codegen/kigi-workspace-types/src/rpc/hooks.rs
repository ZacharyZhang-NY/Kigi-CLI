...
//!
//! `kigi_hooks` pulls in `git2`/`reqwest`/`kigi-tools`, too heavy for
...
/// Wire mirror of `kigi_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `kigi_hooks::config::HookSpec`.
///
...
/// `kigi-hooks`, so a server-side test (`kigi-workspace`'s
...
/// Wire mirror of `kigi_hooks::event::HookEventName`.
///
