...
//! `kigi-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `kigi-workspace-types` (instead of `kigi-workspace` or
/// `kigi-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `kigi_workspace::permission`
