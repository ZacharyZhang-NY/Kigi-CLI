...
//! no I/O, no actor state. They live in `kigi-chat-state` so that both
//! this crate and `kigi-shell` can share them without duplication.
use kigi_sampling_types::{ContentPart, ConversationItem, ToolResultItem};
...
/// `ActiveSubagentSummary` in kigi-tools. The mapping between them
/// happens in `run_compact_inner()` (kigi-shell).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (kigi-tools), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Mirrors `TodoStatus::tag()` in kigi-tools.
...
/// handled by the consumer (e.g. `kigi-shell`), which has access to
...
/// `kigi-chat-state` and `kigi-shell` share the same format.
...
/// `run_compact_inner` in `kigi-shell` assembles inline, but is
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = kigi_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = kigi_sampling_types::repair_dangling_tool_calls(
        items,
        kigi_sampling_types::DanglingToolCallReason::HarnessHalted {
...
actual user question";
    ...
    fn call(id: &str) -> kigi_sampling_types::ToolCall {
    ...
    async fn build_compacted_history_transcript_hint() {
        ...
        let summary = build_compacted_history(input(Some(
            "/home/user/.kigi/sessions/abc/updates.jsonl".to_string(),
        )))
        .last()
        .unwrap()
        .text_content();
        assert!(summary.contains("/home/user/.kigi/sessions/abc/updates.jsonl"));
    ...
    ///
    /// Simulates the exact conversation shape produced by kigi-shell:
    ///
...
The user asked to read main.rs and lib.rs. main.rs prints hello world, lib.rs has an add function. The user then asked to fix a typo in main.rs and run tests. The typo was fixed and tests passed.";
    ...
    fn conversation_item_preserves_reasoning_siblings() {
        use kigi_sampling_types::{AssistantItem, rs};
        let result = strip_tool_messages_for_conversation_item(vec![
            ConversationItem::system("system"),
    ...
    fn prepare_for_summarization_drops_reasoning_sibling_on_mutated_assistant() {
        use kigi_sampling_types::{AssistantItem, ToolCall, rs};
        let mk_reasoning = || {
            ConversationItem::Reasoning(rs::ReasoningItem {
                id: "r_123".to_string(),
                summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
    ...
    fn verbatim_reasoning_kept_unless_messages_backend() {
        use kigi_sampling_types::{ToolCall, rs};
        let mk = || {
            vec![
                ConversationItem::system("sys"),
                ConversationItem::Reasoning(rs::ReasoningItem {
