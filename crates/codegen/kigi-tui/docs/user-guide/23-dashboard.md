...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.kigi/config.toml` under `[keybindings]` like every other shortcut.
...
Set `KIGI_AGENT_DASHBOARD=0` to force-disable the feature for a single
