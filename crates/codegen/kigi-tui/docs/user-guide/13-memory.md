...
3. `KIGI_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.kigi/memory/MEMORY.md` | Global | Facts that apply across all your projects |
...
Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.kigi/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.
...
You can edit memory files directly under `~/.kigi/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
| `watcher.enabled` | `true` | Watch `~/.kigi/memory/` for external edits and reindex |
...
```bash
ls ~/.kigi/memory/
cat ~/.kigi/memory/MEMORY.md
$EDITOR ~/.kigi/memory/MEMORY.md
```
