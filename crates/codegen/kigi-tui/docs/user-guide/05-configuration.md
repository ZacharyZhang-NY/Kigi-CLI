...
2. **Environment variables** (e.g., `XAI_API_KEY`, `KIGI_MEMORY`)
3. **config.toml** (`~/.kigi/config.toml`)
...
The setting can also be overridden with the `KIGI_DEFAULT_SELECTED_PERMISSION`
...
`KIGI_SCROLL_SPEED`, `KIGI_SCROLL_MODE`, `KIGI_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `KIGI_SCROLL_LINES`. Precedence: env var →
...
`KIGI_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
MCP servers can also be configured per-project in `.kigi/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.kigi/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.kigi/config.toml` (current dir) > `<repo-root>/.kigi/config.toml` > `~/.kigi/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `KIGI_MEMORY=1`).
...
`$KIGI_EVENT`, `$KIGI_MESSAGE`, and `$KIGI_SESSION_ID`.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= KIGI_EXTERNAL_OTEL)
...
| `KIGI_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `KIGI_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `KIGI_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `KIGI_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `KIGI_OIDC_ISSUER` | OIDC issuer URL |
| `KIGI_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `KIGI_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `KIGI_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `KIGI_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `KIGI_AGENT` | Custom agent definition path or name |
| `KIGI_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `KIGI_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `KIGI_LOG_FILE` log and headless stderr output |
...
| `KIGI_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `KIGI_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `KIGI_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `KIGI_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.kigi/config.toml` | Main configuration file |
| `~/.kigi/pager.toml` | TUI appearance configuration |
...
| `~/.kigi/lsp.json` | LSP server configuration (user-scoped) |
| `~/.kigi/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.kigi/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.kigi/` within your repository:

| File | What it configures |
|------|--------------------|
| `.kigi/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.kigi/config.toml`) |
| `.kigi/skills/` | Project-specific skills |
| `.kigi/hooks/` | Project-specific lifecycle hooks |
| `.kigi/agents/` | Project-specific agent definitions |
| `.kigi/lsp.json` | LSP server configuration |
| `.kigi/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.kigi/lsp.json` | All projects |
| Project | `.kigi/lsp.json` | Current repository |
...
1. **Project** -- `.kigi/lsp.json`
2. **User** -- `~/.kigi/lsp.json`
