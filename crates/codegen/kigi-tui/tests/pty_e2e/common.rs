...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("KIGI_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
/// (`KIGI_TEST_VERSION`) and an explicit `KIGI_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/KIGI_SHARE_DIR
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    ...
    // auto-trust.
    env.push(("KIGI_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set KIGI_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("KIGI_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".kigi")
        .join(kigi_workspace::trust::TRUST_FILE_NAME);
    let store = kigi_workspace::trust::TrustStore::load_from(store_path);
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&kigi_home).expect("create fake KIGI_SHARE_DIR");
    let config = format!(
...
/// Seed `~/.kigi/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{KIGI_SHARE_DIR|HOME}/.kigi/config.toml` location
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let kigi_home = content.home().join(".kigi");
    std::fs::create_dir_all(&kigi_home).expect("create .kigi");
    // Minimal opt-in only — matches load_config's `{KIGI_SHARE_DIR|HOME}/.kigi/config.toml`.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("KIGI_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "KIGI_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        kigi_tui::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
pub(crate) fn run_wrap(wrap_args: &[&str], extra_env: &[(&str, &str)]) -> (Option<u32>, String) {
    ...
    let mut env: Vec<(&str, &str)> = vec![("KIGI_SHARE_DIR", &home_str), ("NO_COLOR", "1")];
...
/// Dump an asciinema cast of `harness` into `$KIGI_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("KIGI_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use kigi_pager_pty_harness::host_clipboard::{
...
pub(crate) use kigi_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
