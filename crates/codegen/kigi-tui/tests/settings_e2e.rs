...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    kigi_tui::views::settings_modal::render_settings_modal(&mut buf, area, &mut s, false, None);
...
fn defaults_round_trip_through_registry() {
    use kigi_tui::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    kigi_tui::appearance::cache::set_keep_text_selection(
        kigi_tui::appearance::TextSelection::Flash,
    );
    kigi_tui::appearance::cache::set_show_thinking_blocks(true);
    kigi_tui::appearance::cache::set_prompt_suggestions(true);
    kigi_tui::appearance::cache::set_group_tool_verbs(true);
    kigi_tui::appearance::cache::set_scroll_mode(kigi_tui::appearance::ScrollMode::Auto);
    kigi_tui::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    kigi_tui::appearance::cache::set_scroll_lines(3);
    ...
    for meta in reg.all() {
        ...
        let default_value = kigi_tui::settings::default_value_for(meta);
...
fn d_key_emits_open_reset_confirm_for_every_setting() {
    ...
    for meta in reg.all() {
        ...
        let mut s = make_state();
        // A row can be conditionally hidden from the top-level list. Skip
        // settings with no visible row; their reset path is covered by the
...
// `crates/codegen/kigi-tui/src/app/dispatch.rs::tests` (next to
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use kigi_tui::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    use kigi_tui::views::settings_modal::ResetConfirmOverlay;
    use ratatui::buffer::Buffer;
    ...
    kigi_tui::views::settings_modal::render_settings_modal(
        &mut buf,
        area,
        &mut s,
        false,
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    kigi_tui::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use kigi_shell::agent::config::UiConfig;
    use kigi_tui::settings::{PagerLocalSnapshot, SettingsRegistry};
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use kigi_shell::agent::config::UiConfig;
    use kigi_tui::settings::{PagerLocalSnapshot, SettingsRegistry};
    use kigi_tui::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        kigi_tui::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        kigi_tui::settings::default_value_for(mt_meta),
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use kigi_tui::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use kigi_tui::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use kigi_tui::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use kigi_tui::appearance::TextSelection;
    ...
    kigi_tui::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    kigi_tui::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    kigi_tui::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    kigi_tui::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    kigi_tui::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    kigi_tui::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    kigi_tui::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    kigi_tui::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
