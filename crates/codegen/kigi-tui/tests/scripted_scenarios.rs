...
//!
//! These tests exercise the real `kigi-tui` binary through a PTY using
...
/// enterprise deploy report: with `KIGI_GOAL=1`, `/goal` must
...
/// Full folder-trust session: with `KIGI_FOLDER_TRUST=1` and a git repo that
...
/// also pins `KIGI_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
