...
use kigi_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<kigi_shell::util::config::RemoteSettings>,
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = kigi_shell::util::kigi_home::kigi_home().join("config.toml");
...
mod tests {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.kigi/config.toml` with a
