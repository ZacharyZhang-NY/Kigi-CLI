...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: kigi_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: kigi_workspace::foreign_sessions::EnabledForeignSessionSources,
        kigi_home: std::path::PathBuf,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.kigi/config.toml`.
    ...
    /// Persist a typed setting to `~/.kigi/config.toml`. On failure,
    ...
    /// registry (`~/.kigi/active_sessions.json`).
...
pub enum TaskResult {
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<kigi_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    SwitchModelComplete {
        ...
        model_id: acp::ModelId,
        effort: Option<ReasoningEffort>,
        /// Ok carries the effective context window the shell reported
        /// (`totalContextTokens` in the response meta; `None` from a shell
        /// that predates it).
    ...
    HooksActionResult {
        agent_id: AgentId,
        result: Result<kigi_hooks_plugins_types::ActionOutcome, String>,
    },
