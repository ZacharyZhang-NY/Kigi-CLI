...
use kigi_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, kigi_home: &Path) -> [PathBuf; 2] {
        ...
        [
            kigi_home
                ...
                .join(skill)
                .join("SKILL.md"),
            kigi_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(kigi_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, kigi_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, kigi_home, metadata_exists).await;
...
pub(crate) fn scan_effect(
    ...
    (compat.claude || compat.codex || compat.cursor).then(|| Effect::ScanForeignSessions {
        cwd: cwd.to_path_buf(),
        compat,
        kigi_home: kigi_home.to_path_buf(),
        coordinator,
        seq,
    })
}
...
mod tests {
    ...
    use kigi_workspace::foreign_sessions::ForeignSessionSource;
