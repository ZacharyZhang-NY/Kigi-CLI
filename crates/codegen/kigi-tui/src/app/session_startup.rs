...
pub fn fork_session_params(
    ...
    let source_cwd = kigi_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = kigi_shell::util::kigi_home::kigi_home().join("sessions");
    let encoded = kigi_shell::util::kigi_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    kigi_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = kigi_shell::session::persistence::list_summaries(Some(cwd)).await?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &kigi_shell::agent::config::Config,
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if kigi_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = kigi_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = kigi_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
    ...
    let agent_config = kigi_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use kigi_shell::agent::session_registry_client::SessionRegistryClient;
    use kigi_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    ...
    let progress: kigi_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        let encoded = kigi_shell::util::kigi_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = kigi_shell::util::kigi_home::kigi_home()
            .join("sessions")
            .join(&encoded);
