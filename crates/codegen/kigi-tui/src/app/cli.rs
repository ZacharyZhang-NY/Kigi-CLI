...
Examples:
  kigi wrap docker exec -it my-container bash
  kigi wrap kubectl exec -it my-pod -- bash

See ~/.kigi/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `~/.kigi/config.toml` or when the `KIGI_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "KIGI_AGENT_SECRET")]
...
#[command(
    ...
    about = "Kigi — unofficial Kimi Code CLI community build",
...
pub struct PagerArgs {
    ...
    /// persists per-segment markdown to grep. Sets `KIGI_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            kigi_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `kigi -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "KIGI_SANDBOX", value_name = "PROFILE")]
    ...
    /// Session-scoped only — does not write config. To default plain `kigi` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.kigi/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.kigi/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "KIGI_LOG_SAMPLING", hide = true)]
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`KIGI_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                kigi_shell::session::persistence::resumed_session_sandbox_profile(Some(&id), cwd)
            }
            ResumeTarget::MostRecentForCwd => {
                kigi_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<kigi_sandbox::ProfileName>().ok()
                    != s.parse::<kigi_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `kigi "<prompt>"` launch form; the headless
