...
use kigi_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> kigi_shell::extensions::notification::ImageCompressedEntry {
    kigi_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use kigi_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
/// Shared temp `KIGI_SHARE_DIR` for disk-replay tests. `kigi_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `KIGI_SHARE_DIR`
...
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    ...
    crate::app::subagent::set_replay_kigi_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_kigi_home_for_tests(None);
    out
}
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(kigi_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = kigi_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: kigi_shell::extensions::mcp::McpServerStatus,
