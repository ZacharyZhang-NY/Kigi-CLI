#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn queue_changed_adopts_running_prompt_id_only_when_unset() {
        // Shell and pager share the kigi-prompt-queue type, so serializing the payload as the
        // shell emits it pins the wire shape the handler consumes, not cross-crate compat.
        let shell_payload = kigi_shell::session::prompt_queue::QueueChanged {
    ...
    fn adopted_running_prompt_lets_subsequent_chunk_through_gate() {
        ...
        let msg = AcpClientMessage::SessionNotification(kigi_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
        handle(msg, &mut app);
    ...
    fn bash_kind_round_trips_and_adoption_sets_bash_turn() {
        // Shell and pager share the kigi-prompt-queue type; pin kind through a serde cycle.
        let shell = kigi_shell::session::prompt_queue::QueueChanged {
            session_id: "sess-1".to_string(),
            entries: vec![kigi_shell::session::prompt_queue::QueueEntryWire {
                id: "b1".to_string(),
                version: 0,
                owner: None,
                last_editor: None,
