use super::*;
use kigi_shell::sampling::error::rate_limited_user_message;
    ...
    let changed = match session_notif.update {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        kigi_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        ...
                        kigi_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
        ...
        XaiSessionUpdate::HooksChanged {
            ...
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(kigi_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(kigi_token_estimation::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[kigi_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &kigi_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use kigi_tools::types::SessionMode;
