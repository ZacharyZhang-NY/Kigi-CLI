...
pub use kigi_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `KIGI_MOUSE_REPORTING_TOGGLE`). Seeded once
...
///
/// Uses [`kigi_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `kigi-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`kigi-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub async fn run(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(kigi_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = kigi_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("KIGI_HUNK_TRACKER").ok();
...
fn disable_mouse_paste_raw() {
    kigi_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(kigi_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    kigi_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        if mode.is_fullscreen() {
            kigi_shell::util::with_locked_stderr(|stderr| execute!(stderr, EnterAlternateScreen))?;
        }
        #[cfg(windows)]
        if want_minimal {
        ...
        kigi_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(kigi_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                kigi_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = kigi_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let backend =
                CrosstermBackend::new(crate::render::draw::TermWriter::new(frame_tx, writer_sync));
            let term = kigi_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
...
mod tests {
    ...
    #[test]
    fn cli_command_name_is_kigi() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "kigi");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "",
                "Usage: kigi [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
