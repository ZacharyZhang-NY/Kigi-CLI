...
fn effective_kigi_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_KIGI_SHARE_DIR.with(|h| h.borrow().clone()) {
        return home;
    }
...
pub(crate) fn enrich_from_meta(
    ...
    enrich_from_meta_with_home(info, &effective_kigi_home(), parent_cwd, parent_session_id);
}
...
pub(crate) fn replay_inherited_updates(
    ...
    let home = effective_kigi_home();
    let updates =
        match kigi_shell::session::storage::load_updates_for_replay_at(child_session_id, &home) {
            Ok(Some(u)) => u,
            Ok(None) => return,
            Err(e) => {
                tracing::debug!(
...
mod tests {
    ...
    fn ensure_subagent_child_replayed_releases_retained_memory_once() {
        ...
        set_replay_kigi_home_for_tests(Some(home.path().to_path_buf()));
        let mut parent = make_min_child_view();
        parent
            .subagent_views
        ...
        set_replay_kigi_home_for_tests(None);
    }
    #[test]
