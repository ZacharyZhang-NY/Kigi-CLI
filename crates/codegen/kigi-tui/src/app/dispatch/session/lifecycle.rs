...
pub(in crate::app::dispatch) fn open_agent_type_mismatch_question(
    ...
    effort: Option<kigi_shell::sampling::types::ReasoningEffort>,
    model_name: &str,
) -> Vec<Effect> {
    use crate::views::question_view::{LocalQuestionKind, QuestionViewState};
    use kigi_tools::implementations::kigi::ask_user_question::{Question, QuestionOption};
...
/// the workspace (writes `~/.kigi/trusted_folders.toml`), mark trust resolved,
...
pub(in crate::app::dispatch) fn dispatch_trust_folder(app: &mut AppView) -> Vec<Effect> {
    if let TrustState::Pending { workspace } = &app.trust_state {
        kigi_workspace::folder_trust::grant_folder_trust(workspace);
    }
    finish_trust(app)
}
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        let mut effects = match result {
            Ok(context_window) => {
                ...
                if let Some(total) = context_window {
                    agent.apply_context_window(total);
                }
