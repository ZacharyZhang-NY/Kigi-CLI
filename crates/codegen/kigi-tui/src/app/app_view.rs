...
/// [`kigi_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<kigi_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: kigi_shell::util::config::WorktreeHintMode) -> Self {
        use kigi_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        kigi_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) = kigi_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`kigi_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `KIGI_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `KIGI_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("KIGI_ESC_DOUBLE_PRESS_MS").ok()))
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: kigi_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`kigi_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.kigi/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`KIGI_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// non-selectable headers. Gated by `KIGI_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `KIGI_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `kigi-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<kigi_shell::extensions::session_search::SearchSessionHit>>,
    ...
    /// `kigi_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `KIGI_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: kigi_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<kigi_shell::util::config::ContextualHintsRemote>,
    ...
    /// `~/.kigi/config.toml`. `None` when the file/section is absent
...
impl AppView {
    ...
    pub fn new(
        ...
        Self {
            ...
            auto_mode_gate: kigi_shell::util::config::auto_permission_mode_enabled_from_disk(),
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<kigi_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    /// gate flips, startup, reconnect, and session create/switch (so new agents
    /// inherit the gate).
    ...
    /// Rows the dev `KIGI_FPS` overlay occupies (0 in non-dev builds), so
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: kigi_shell::util::config::ResolvedContextualHints,
    ) {
...
pub(crate) mod tests {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![kigi_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: kigi_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(kigi_workspace::foreign_sessions::RecentForeignSession {
    ...
    fn pending_menu_items_lists_interactive_methods_plus_quit() {
        ...
        assert_eq!(items.len(), 14, "13 login rows + Quit, got {items:?}");
        assert!(
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![kigi_workspace::file_system::FuzzyMatchResult {
