...
pub(crate) const KIGI_SCREEN_MODE_ENV: &str = "KIGI_SCREEN_MODE";
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    // Keep a CLI mode flag for hand-pasted resume hints that omit KIGI_SCREEN_MODE.
    if want_minimal {
...
pub(crate) fn exec_screen_mode_relaunch(session_id: &str, want_minimal: bool) -> io::Result<()> {
    ...
    cmd.env(KIGI_SCREEN_MODE_ENV, screen_mode_env_value(want_minimal));
...
/// Parse a [`KIGI_SCREEN_MODE_ENV`] or config `[ui] screen_mode` value
...
/// Consume the one-shot screen-mode override env (see [`KIGI_SCREEN_MODE_ENV`]).
///
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(KIGI_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(KIGI_SCREEN_MODE_ENV) };
    }
...
mod tests {
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry KIGI_SCREEN_MODE so following the
