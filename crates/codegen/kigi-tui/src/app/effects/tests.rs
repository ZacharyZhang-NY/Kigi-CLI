...
fn parse_worktree_restore_payload_full() {
    use kigi_workspace::session::git::RestoreDegree;
...
/// Redirect `KIGI_SHARE_DIR` to a tempdir for test isolation.
fn setup_kigi_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("KIGI_SHARE_DIR", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use kigi_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        kigi_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        kigi_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_kigi_home_in_tempdir();
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: kigi_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            kigi_home: PathBuf::from("/path/that/must/not/be-read"),
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    use kigi_acp_lib::AcpAgentMessage;
...
/// Neutralize `KIGI_AGENT` for the profile-matrix tests below: agent-driven
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> kigi_shell::session::SessionInfoResponse {
    use kigi_shell::session::acp_types::{ContextInfo, SessionInfoData};
    kigi_shell::session::SessionInfoResponse {
