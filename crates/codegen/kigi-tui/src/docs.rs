...
static REFERENCE_DOCS: &[Doc] = &[
    Doc {
        ...
        description: "Using hooks and plugins",
...
/// Extract user-guide docs to `<kigi_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(kigi_home: &std::path::Path) {
    let docs_dir = kigi_home.join("docs").join("user-guide");
