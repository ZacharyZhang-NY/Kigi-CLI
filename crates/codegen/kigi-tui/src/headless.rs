...
use kigi_acp_lib::{AcpAgentTx, AcpClientMessageBox, acp_send};
use kigi_shell::agent::auth_method::AuthMethodKind;
use kigi_shell::agent::config::Config as AgentConfig;
use kigi_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
use kigi_shell::sampling::error::{RATE_LIMITED_ERROR_CODE, rate_limited_user_message};
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<kigi_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use kigi_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(json: &str) -> anyhow::Result<Vec<kigi_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = kigi_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut kigi_shell::agent::config::Config) {
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive =
                std::io::stdin().is_terminal() && !kigi_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &kigi_tools::types::compat::CompatConfig::default());
...
pub async fn run_single_turn(
    ...
    agent_config.mode = kigi_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = kigi_shell::agent::config::CliAgentOverrides {
    ...
    let track_active = std::env::var("KIGI_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = kigi_shell::active_sessions::register(kigi_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(kigi_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(kigi_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    match prompt_result {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = kigi_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    notif: &kigi_acp_lib::AcpArgsBox<acp::ExtNotification>,
    format: OutputFormat,
) -> ExtEvent {
...
mod tests {
    ...
    use super::*;
    use kigi_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            kigi_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> kigi_acp_lib::AcpArgsBox<acp::ExtNotification> {
