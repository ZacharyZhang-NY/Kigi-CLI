...
use kigi_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use kigi_shell::agent::config::AgentSelectionConfig;
...
/// User-level vs project-level config files (`~/.kigi` vs `{cwd}/.kigi`).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = kigi_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.kigi/personas` and `{cwd}/.kigi/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = kigi_home.join("bundled").join("personas");
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".kigi").join("personas")),
        (ConfigFileScope::User, kigi_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match kigi_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => kigi_config::kigi_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".kigi").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.kigi/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.kigi/{subdir}` or `{cwd}/.kigi/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let kigi_home = kigi_config::kigi_home();
    let in_user = dunce::canonicalize(kigi_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".kigi").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    kigi_shell::config::load_effective_config()
        .ok()
        .and_then(|root| kigi_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in kigi-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    kigi_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn set_default_agent(name: Option<&str>) -> Result<(), String> {
    let config_path = kigi_config::kigi_home().join("config.toml");
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        kigi_agent::config::PromptMode::Extend => "extend",
        kigi_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use kigi_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".kigi").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".kigi").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.kigi/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".kigi").join("personas").join("gone.toml");
