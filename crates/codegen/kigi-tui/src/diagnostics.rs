...
pub fn summarize_warnings(warnings: &[TerminalWarning]) -> Option<crate::startup::StartupWarning> {
    ...
    let is_ssh = kigi_shell::util::clipboard::is_remote_session();
...
pub fn collect_startup_warnings(
    ...
    if ctx.brand == TerminalName::AppleTerminal && kigi_shell::util::clipboard::is_remote_session()
    {
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(kigi_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        message: format!(
            "Your project sandbox profile conflicts with user config.\nProfile: {profiles}\nProject config: .kigi/sandbox.toml\nUser config: ~/.kigi/sandbox.toml"
        ),
...
pub fn diagnose_wayland_data_control_live() -> Option<TerminalWarning> {
    ...
    diagnose_wayland_data_control(
        is_wayland,
        is_wayland && kigi_shell::util::clipboard::wayland_data_control_supported(),
        is_wayland && kigi_shell::util::clipboard::native_tool_name() == "wl-copy",
    )
}
