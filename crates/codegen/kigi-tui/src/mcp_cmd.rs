...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.kigi/config.toml`, available in all your projects
    User,
    /// `./.kigi/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.kigi/config.toml) or project (./.kigi/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let cwd = current_dir_or_exit();
    let servers = kigi_shell::util::config::load_mcp_server_configs_with_project(&cwd);
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    kigi_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        ...
        McpScope::Project => kigi_shell::util::config::project_config_path(&current_dir_or_exit()),
    }
}
...
fn select_remove_site(
    ...
    use kigi_shell::util::config::user_config_path;
...
fn surviving_definition(
    ...
    project_site
        .map(|path| (McpScope::Project, path))
        .or_else(|| {
            user_defined.then(|| (McpScope::User, kigi_shell::util::config::user_config_path()))
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        kigi_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    // ancestor .kigi/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = kigi_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        kigi_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        // Scheme-less URL-looking commands stay stdio (no inference) and the
        // warning suggests a command that passes URL validation verbatim.
        ...
        // Extra args mean a command is being described — no inference even
        // with an http(s) scheme.
        let add = parse_add(&[
            "kigi",
            "mcp",
            "add",
    ...
    fn add_infers_http_for_bare_positional_url() {
        for url in ["https://mcp.example.com/mcp", "http://mcp.example.com/mcp"] {
            let add = parse_add(&["kigi", "mcp", "add", "api", url]);
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = kigi_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.kigi/config.toml");
