...
/// handle `~/.kigi/worktrees/<repo>/<session>/`) and calls `remove_worktree()`
...
fn try_btrfs_remove(
    ...
    if btrfs_info.bind_mount_source.is_some() {
        ...
        kigi_tty_utils::detach_std_command(&mut umount_cmd);
...
pub mod gc {
    ...
    /// symlinked `$KIGI_SHARE_DIR` or custom worktree path would otherwise never match.
...
mod tests {
    ...
    fn test_cleanup_worktrees_in_with_nested_dirs() {
        ...
        // Create ~/.kigi/worktrees/<repo>/<session>/ structure.
        let worktrees_dir = tmp.path().join("worktrees");
    ...
    fn test_cleanup_worktrees_in_removes_nested_dangling_symlink() {
        // Dangling symlink one level deeper (~/.kigi/worktrees/<repo>/<session>):
    ...
    fn delete_fallback_invokes_delegate_when_direct_delete_fails() {
        ...
        let report = delete_snapshot_with_delegate_fallback(
            ...
            Path::new("/home/u/.kigi/worktrees/repo/wt"),
            Some(&delegate),
    ...
    fn test_cleanup_orphaned_btrfs_skips_when_mount_target_parent_missing() {
        ...
        let mount_target = unrestored_home.join(".kigi/worktrees/x/wt-live");
        assert!(
    ...
    fn test_cleanup_orphaned_btrfs_skips_active() {
        ...
        let mount_target = std::path::PathBuf::from("/home/user/.kigi/worktrees/active-wt");
