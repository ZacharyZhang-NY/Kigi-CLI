...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("KIGI_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = kigi_config::shell::shell_command_argv(&command_str);
    ...
    // plugin) can spoof `KIGI_HOOK_EVENT`, `KIGI_HOOK_NAME`, `KIGI_SESSION_ID`,
    // `KIGI_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR` -- which are the
    ...
    let mut child = match cmd
        ...
        .env("KIGI_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("KIGI_HOOK_NAME", &spec.name)
        .env("KIGI_SESSION_ID", ctx.session_id)
        .env("KIGI_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `KIGI_WORKSPACE_ROOT`; native `.kigi` hooks should use
        // `KIGI_WORKSPACE_ROOT`.
...
mod tests {
    ...
    fn resolve_relative_path() {
        let spec = HookSpec {
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/project/.kigi/hooks"),
            extra_env: std::collections::HashMap::new(),
        };
        assert_eq!(
            resolve_command_path(&spec),
            Some(std::path::PathBuf::from(
                "/project/.kigi/hooks/bin/check.sh"
            ))
        );
    }
    ...
    fn find_unresolved_skips_runner_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${KIGI_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${KIGI_SESSION_ID}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.kigi-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".kigi-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.kigi-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.kigi-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.kigi-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
