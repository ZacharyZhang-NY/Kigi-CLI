//! Integration tests for kigi-hooks.
//!
...
use kigi_hooks::discovery::load_hooks;
use kigi_hooks::dispatcher;
use kigi_hooks::event::*;
use kigi_hooks::result::HookDecision;
use kigi_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        kigi_hooks::result::HookRunResult::Success { .. }
    ));
}
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$KIGI_HOOK_EVENT" > {f}; echo "NAME=$KIGI_HOOK_NAME" >> {f}; echo "SESSION=$KIGI_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`KIGI_HOOK_EVENT`, `KIGI_HOOK_NAME`,
/// `KIGI_SESSION_ID`, `KIGI_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$KIGI_HOOK_EVENT" > {f}; echo "NAME=$KIGI_HOOK_NAME" >> {f}; echo "SESSION=$KIGI_SESSION_ID" >> {f}; echo "ROOT=$KIGI_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "KIGI_HOOK_EVENT": "spoofed_event",
                                "KIGI_HOOK_NAME": "spoofed_name",
                                "KIGI_SESSION_ID": "spoofed_session",
                                "KIGI_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "KIGI_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed KIGI_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "KIGI_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed KIGI_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "KIGI_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed KIGI_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use kigi_hooks::result::HookRunResult;
    let info = match run {
