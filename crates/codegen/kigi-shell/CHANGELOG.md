...
- **MCP tool output truncation** can now be set per-repo in `.kigi/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.kigi/worktrees/.
...
- **Plan files** now default to.kigi/plan.md to match Grok conventions.
...
- **Agent-run terminal commands** now set `KIGI_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, KIGI_HUNK_TRACKER, or config.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.kigi.
...
- **Web search and X search** can now execute server-side with full session persistence when KIGI_BACKEND_SEARCH is enabled.
...
- **Goal mode** is now opt-in via config.toml or the KIGI_GOAL environment variable.
...
- **Goal mode** is now opt-in via `[features] goal = true` in config.toml or the KIGI_GOAL environment variable.
...
- **AI shell command suggestions** enabled via KIGI_SUGGESTIONS_AI=1 environment variable.
...
- **Computer Hub integration** into kigi-shell
...
- **Model lists and pickers** now show only models accessible via your `KIGI_CODE_XAI_API_KEY`.
...
- **`grok models`** and TUI picker now show team-specific models when **`KIGI_CODE_XAI_API_KEY`** is set.
...
- **Import Claude settings** from `.claude/` into `.kigi/config.toml` via welcome screen ('i') or `/import-claude`.
...
- **Unified logs** now saved to `~/.kigi/logs/unified.jsonl` with **Download Logs** in desktop.
...
- **Bundled agents** discovered from ~/.kigi/bundled/agents/ with lowest precedence after project/user/built-in.
...
- **Enterprise managed configs** via ~/.kigi/managed_config.toml/requirements.toml with `grok inspect` and `grok setup`.
- **web_fetch tool** enabled via [features] web_fetch in config.toml alongside env/remote settings.
- **grok login defaults to OAuth** instead of legacy relay; use --legacy or KIGI_OAUTH_ENABLED=0 for old behavior.
...
- **Per-hook enable/disable toggles** via ~/.kigi/disabled-hooks file and 'e' key with **multi-line j/k navigation fix**.
...
- **Parallel tool dispatch** via KIGI_PARALLEL_TOOL_DISPATCH=1 runs concurrent Phase 2 execution.
...
- **OAuth login option** via --oauth/--legacy flags, KIGI_OAUTH_ENABLED env, feature flag (legacy default).
...
- **LSP integration** injects diagnostics as reminders and exposes opt-in code intelligence tools via `.kigi/lsp.json`.
...
- **Image generation tool** gated behind a server-side flag or `KIGI_IMAGE_GEN=1` env var with session auth.
- **Dynamic nested skill discovery** in `.kigi/skills/` subdirs with runtime system reminder announcements.
...
- **Configurable permission policies** enable automatic allow/deny rules via ~/.kigi/config.toml before existing checks.
...
- **Skills discovery expands** to.agents/skills directories alongside.kigi and.claude.
...
- **Dynamic discovery of nested skills** from.kigi/skills/ subdirs at runtime via reminders.
...
- **Discover skills from**.claude/skills directories alongside.kigi/skills.
...
- **ACP x.ai/auth/logout** removes scopes from ~/.kigi/auth.json.
- **Custom npm registry** via config.toml or KIGI_NPM_REGISTRY respects enterprise.npmrc.
...
- **Help skill reads ~/.kigi/config.toml** to answer MCP server and model configuration queries.
...
- **Bundled /help skill** extracts README.md to ~/.kigi/ for slash command and model self-help.
...
- **Subagent persona support** enables layered instructions from config.toml and.kigi/personas/*.toml with fail-closed resolution.
...
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP logout method** removes scopes from ~/.kigi/auth.json.
- **Discovers skills from.claude/skills** alongside.kigi/skills across local, repo, and user directories.
...
- **Post-A/B session hooks** via KIGI_AB_POST_SESSION_HOOK capture worktree changes.
...
- **Enhanced announcement UX** adds paging, prev/next commands, and `KIGI_DEV_ANNOUNCEMENTS` override.
...
- **Optional auth for feedback** enables unauthenticated submissions with `KIGI_USER_METADATA`.
...
- **Independent feedback gating** via KIGI_FEEDBACK_ENABLED separates it from telemetry controls.
...
- **User-defined subagents** via .kigi/agents/*.md files with per-subagent config toggles and dynamic Task tool descriptions.
...
- **User-defined subagents** via.kigi/agents/*.md with config toggles.
...
- **Disables web search tool** via `--disable-web-search` flag or `KIGI_DISABLE_WEB_SEARCH=1` for benchmarks.
...
- **Unauthenticated feedback submission** via optional tokens and `KIGI_USER_METADATA` env.
...
- **Hooks system** executes custom scripts for pre/post-tool and session events from ~/.kigi/hooks/ with deny-wins trust controls.
- **Independent feedback flag** gates /feedback and popups separately from telemetry via KIGI_FEEDBACK_ENABLED.
...
- **Subagent support** gated behind `--subagents` flag, `KIGI_SUBAGENTS` env var, config.toml, or remote settings — disabled by default.
...
- **Replay notifications now guaranteed before session/load response** via `forward_with_completion()` drain; removes public `acp_send_fire_and_forget` from kigi-acp-lib.
...
- **`KIGI_MEMORY=0` force-disables memory** regardless of config.toml or remote settings; CLI flag still overrides.
...
- **Leader auto-update pre-downloads the binary** before shutdown and resolves the ~/.kigi/bin symlink at spawn time.
...
- **Sandbox profile configurable via KIGI_SANDBOX env variable**, defaulting to workspace profile on devboxes.
...
- **Agent environment metadata becomes queryable** by merging KIGI_AGENT_METADATA into initialize responses and persisting the full blob in backend agent records.
...
- **npm installs now use a canonical Grok binary path** via postinstall copy to `~/.kigi/bin/grok`, preventing installer conflicts and version confusion.
