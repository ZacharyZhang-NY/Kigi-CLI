...
use kigi_shell::cpu_profile::ControlErrorCode;
...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, kigi_shell::leader::ServerHandle) {
...
async fn test_session_ownership_cleanup_on_disconnect() {
    use kigi_shell::leader::run_leader_server;
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(kigi_shell::leader::ShutdownReason::Manual);
...
async fn test_version_mismatch_notification_sent_to_client() {
    use kigi_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, run_leader_server};
    use std::sync::Arc;
    use std::sync::atomic::{AtomicBool, AtomicUsize};
    use tokio::sync::{mpsc, watch};
    use tokio_util::sync::CancellationToken;
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use kigi_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != kigi_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        kigi_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
///
/// This binary does not sandbox KIGI_SHARE_DIR, so on a dev machine these entries
/// land in the real `~/.kigi` log — accepted: the server already writes
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = kigi_log::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        kigi_shell::leader::DisconnectReason::Connected,
