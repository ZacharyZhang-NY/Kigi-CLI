...
use kigi_shell::session::storage::JsonlStorageAdapter;
use kigi_test_support::sse::responses_api_reasoning_and_text_events;
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use kigi_shell::sampling::rs::ResponseStreamEvent;
        match event {
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use kigi_shell::sampling::rs::OutputMessageContent;
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use kigi_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
