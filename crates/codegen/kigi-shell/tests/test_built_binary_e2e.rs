...
use kigi_test_support::env::test_env_cmd_tokio;
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        ...
        kigi_test_support::scripted::ScriptedResponse::sse(
            kigi_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
    ...
    fn new(server: &MockInferenceServer) -> Self {
        ...
        Self {
            ...
            env: vec![
                ...
                ("KIGI_INSTRUMENTATION".into(), "disabled".into()),
                ("KIGI_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }
...
"#,
    ...
    .unwrap();
    h.env("KIGI_TEST_BYOK_TOKEN", "test-byok-secret-token");
