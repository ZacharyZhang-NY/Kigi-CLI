...
//! `KIGI_SHARE_DIR` (the `kigi_home` `OnceLock` allows a single value per process)
...
use kigi_shell::config::ServingIdentity;
use kigi_test_support::spawn_counting_server;
...
async fn team_switch_evicts_prior_teams_policy() {
    ...
    assert!(!kigi_shell::config::is_managed_config_stale_for(
...
async fn empty_dk_response_with_failing_team_leaves_team_policy_intact() {
    ...
    let err = kigi_shell::managed_config::sync()
        .await
...
async fn expired_refreshable_team_token_heals_after_auth_refresh() {
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        !kigi_shell::managed_config::has_principal(),
...
async fn managed_policy_gate_fails_closed_on_deleted_policy_offline() {
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        kigi_shell::managed_config::managed_policy_gate().is_err(),
...
async fn bootstrap_fails_closed_when_managed_policy_compromised() {
    ...
    let cfg = kigi_shell::agent::config::Config::default();
    let auth_manager = std::sync::Arc::new(kigi_shell::auth::AuthManager::new(
        &home,
        kigi_shell::auth::GrokComConfig::default(),
    ));
    // `bootstrap`'s Ok type isn't `Debug`, so match rather than `expect_err`.
    let err = match kigi_shell::agent::init::bootstrap(&cfg, &auth_manager, None) {
        Err(e) => e,
        Ok(_) => {
...
/// Live wiring guard: an offline `KIGI_DEPLOYMENT_KEY` switch on a fail_closed install must FAIL CLOSED, else a
...
async fn managed_policy_gate_fails_closed_on_deployment_key_switch_offline() {
    ...
    unsafe { std::env::set_var("KIGI_DEPLOYMENT_KEY", "KEY-AAA") };
    kigi_shell::managed_config::sync()
        .await
    ...
    // KIGI_MANAGED_CONFIG=0 disables any incidental background fetch (the gate is sync anyway).
    // SAFETY: #[serial] test; restored before any assertion below.
    unsafe { std::env::set_var("KIGI_MANAGED_CONFIG", "0") };

    // Same key A → matching fingerprint → ALLOW (exact-equality vs recorded blake3).
    let gate_same_key = kigi_shell::managed_config::managed_policy_gate();

    // Switch to a different key B → REFUSE: exercises the full offline wiring.
    // SAFETY: #[serial] test; restored immediately below.
    unsafe { std::env::set_var("KIGI_DEPLOYMENT_KEY", "KEY-BBB") };
    let gate_switched_key = kigi_shell::managed_config::managed_policy_gate();

    // SAFETY: #[serial] test; restore env BEFORE asserting so a failed assert can't leak it to later tests.
    unsafe {
        std::env::remove_var("KIGI_DEPLOYMENT_KEY");
        std::env::remove_var("KIGI_MANAGED_CONFIG");
    }
...
/// The session-start gate reads no env: `KIGI_MANAGED_CONFIG_FAIL_CLOSED=0` must NOT disarm a fail_closed
...
async fn fail_closed_env_cannot_disarm_the_gate() {
    ...
    unsafe {
        std::env::set_var("KIGI_MANAGED_CONFIG", "0"); // offline (the gate is sync anyway)
        std::env::set_var("KIGI_MANAGED_CONFIG_FAIL_CLOSED", "0"); // attempt a local disarm
    }
    let gate = kigi_shell::managed_config::managed_policy_gate();
    unsafe {
        std::env::remove_var("KIGI_MANAGED_CONFIG");
        std::env::remove_var("KIGI_MANAGED_CONFIG_FAIL_CLOSED");
    }

    assert!(
        gate.is_err(),
        "KIGI_MANAGED_CONFIG_FAIL_CLOSED=0 must not disarm the session-start gate (no local bypass)"
    );
}
...
async fn setup_lock_skip_is_not_reported_as_no_config() {
    ...
    assert!(
        matches!(outcome, kigi_shell::managed_config::SetupOutcome::Installed),
...
async fn shared_client_reuses_pooled_connection() {
    ...
    let client = kigi_shell::http::shared_client();
...
/// `KIGI_MANAGED_CONFIG=0` is an explicit opt-out: the post-login sync must
/// make zero requests.
#[tokio::test]
#[serial]
async fn managed_config_opt_out_makes_no_requests() {
    ...
    unsafe { std::env::set_var("KIGI_MANAGED_CONFIG", "0") };
    let outcome = kigi_shell::managed_config::post_login_sync(None).await;
    unsafe { std::env::remove_var("KIGI_MANAGED_CONFIG") };

    assert_eq!(
        outcome,
        kigi_shell::managed_config::ManagedConfigSync::Skipped
    );
...
async fn post_login_pins_authenticated_team_over_disk() {
    ...
    let pinned: kigi_shell::auth::GrokAuth = serde_json::from_value(serde_json::json!({
        "key": "pinned-token",
        "auth_mode": "oidc",
        "create_time": "2026-01-01T00:00:00Z",
        "expires_at": FAR_FUTURE,
    ...
    let outcome = kigi_shell::managed_config::post_login_sync(Some(pinned)).await;
    assert_eq!(
        outcome,
        kigi_shell::managed_config::ManagedConfigSync::Updated { is_team: true }
    );
    assert_eq!(
        auths.lock().unwrap().last().map(String::as_str),
