...
const KIGI_CODE_BACKEND_URL: &str = "https://code.kigi.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const KIGI_CODE_WEB_URL: &str = "https://grok.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("KIGI_CODE_WEB_URL").unwrap_or_else(|_| KIGI_CODE_WEB_URL.to_string());
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("KIGI_CODE_BACKEND_URL")
                .unwrap_or_else(|_| KIGI_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_static(kigi_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = kigi_file_utils::trace_context::inject_trace_context_into_request(
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(crate::util::agent_id::agent_id)
        .await
        .ok()?;
...
mod tests {
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("KIGI_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
        ...
        assert_eq!(url, "https://api.kimi.com/coding/v1/deployment/config");
        assert!(
        ...
        unsafe { std::env::remove_var("KIGI_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
