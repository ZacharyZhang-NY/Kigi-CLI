...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `KIGI_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("KIGI_WEB_FETCH_PROXY"))
...
mod tests {
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            kigi_agent::config::AgentDefinition::plan(),
            kigi_agent::config::AgentDefinition::explore(),
        ] {
