//! Notification bridge: translates `kigi-tools` `ToolNotification` events
//! into `kigi-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
pub struct NotificationBridgeConfig {
    ...
    pub auto_wake_delivered: kigi_tools::reminders::task_completion::AutoWakeDeliveredIds,
...
async fn emit_current_mode_update(
    ...
    mode: kigi_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor = task_snapshot.kind == kigi_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    kigi_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    kigi_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = kigi_tools::reminders::wrap_reminder(&body);
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, kigi_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
        ...
        ToolNotification::PlanModeExited(exited) => {
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                emit_current_mode_update(config, kigi_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    use super::*;
    use kigi_tools::computer::types::TaskKind;
    use kigi_tools::types::TaskSnapshot;
    ...
    fn make_test_config_full() -> (
        NotificationBridgeConfig,
        mpsc::UnboundedReceiver<kigi_acp_lib::AcpClientMessage>,
        mpsc::UnboundedReceiver<PersistenceMsg>,
        mpsc::UnboundedReceiver<SessionCommand>,
    ) {
        let (gateway_tx, gateway_rx) = mpsc::unbounded_channel();
        let gateway = kigi_acp_lib::AcpAgentGatewaySender::new(gateway_tx);
    ...
    async fn bash_task_completed_injects_bash_task_completed_source() {
        ...
        // Auto-wake sends the synthetic Prompt (not InjectNotification).
    ...
    async fn bash_task_completed_suppresses_auto_wake_during_goal_loop() {
        ...
        // No synthetic prompt / InjectNotification while the goal
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(kigi_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            kigi_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification =
            ToolNotification::BashOutputChunk(kigi_tools::notification::types::BashOutputChunk {
                base: kigi_tools::notification::types::BashNotificationBase {
        ...
        let broadcast_id = match gateway_rx.try_recv().expect("chunk must be broadcast") {
            kigi_acp_lib::AcpClientMessage::SessionNotification(args) => args
                .request
                .meta
                .as_ref()
                .and_then(|m| m.get("eventId"))
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            kigi_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            kigi_tools::notification::types::BashExecutionBackgrounded {
                base: kigi_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            kigi_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification =
            ToolNotification::PlanModeEntered(kigi_tools::notification::types::PlanModeEntered {
