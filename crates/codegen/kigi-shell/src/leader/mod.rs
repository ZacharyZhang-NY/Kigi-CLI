...
//!
//! ```ignore
//! use kigi_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
pub use lock::{
    LEADER_SOCKET_ENV, LeaderLock, LockError, default_lock_path, default_lock_path_in,
    default_socket_path, default_socket_path_in, socket_suffix_from_paths,
};
pub use protocol::{
...
const CLIENT_LEADER_VERSION: &str = kigi_version::VERSION;
...
pub struct LeaderDescriptor {
    ...
    /// `true` for the default instance (empty socket suffix); non-default
    /// instances only arise via [`LEADER_SOCKET_ENV`].
    pub is_default: bool,
...
impl LeaderTargetSelection {
    ...
    pub fn socket_suffix(&self) -> &str {
        &self.descriptor.socket_suffix
    }
...
pub enum LeaderTarget {
    /// The default instance (empty socket suffix).
    Default,
    /// An explicit socket path (matched against the descriptor's socket path).
    Socket(PathBuf),
    Pid(u32),
}
...
fn descriptor_from_paths(
    ...
    let socket_suffix =
        live_info
            .as_ref()
            .map(|info| info.socket_suffix.clone())
            .or_else(|| {
                lock_path.as_deref().zip(socket_path.as_deref()).and_then(
                    |(lock_path, socket_path)| socket_suffix_from_paths(lock_path, socket_path),
                )
            })
            .or_else(|| {
                lock_path
            ...
            .unwrap_or_default();
    let is_default = socket_suffix.is_empty();
    LeaderDescriptor {
        pid_from_lock,
        lock_path,
        socket_path,
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : kigi_version::VERSION,
                            }
                        )),
                    );
                }
...
/// Resolve a non-PID target against the discovered descriptors: exactly one
/// reachable match wins; otherwise the most specific failure is reported.
fn resolve_matching_descriptors(
    target_note: &str,
    matching: Vec<LeaderDescriptor>,
    ...
    if reachable.len() == 1 {
        let Some(descriptor) = reachable.into_iter().next() else {
            return Err(LeaderTargetError::new(
                ...
                format!("no reachable leader found for target {}", target_note),
            ));
        };
    ...
    if reachable.len() > 1 {
        return Err(LeaderTargetError::new(
            ...
            format!("multiple leader candidates matched target {}", target_note),
        ));
    }
    ...
    if matching
        .iter()
        .any(|descriptor| descriptor.target_error == Some(LeaderTargetErrorCode::SocketUnreachable))
    {
        return Err(LeaderTargetError::new(
            LeaderTargetErrorCode::SocketUnreachable,
            format!(
                "leader target for {} has an unreachable socket",
                target_note
            ),
        ));
    }
...
fn resolve_target_from_descriptors(
    ...
    match target {
        ...
        LeaderTarget::Socket(socket_path) => {
            let note = socket_path.display().to_string();
            let matching: Vec<_> = leaders
                .into_iter()
                .filter(|descriptor| descriptor.socket_path.as_deref() == Some(&*socket_path))
                .collect();
            resolve_matching_descriptors(&note, matching)
        }
...
///
/// The socket/lock paths come from [`default_socket_path`] (honoring the
/// [`LEADER_SOCKET_ENV`] override), so all clients on a machine share one
/// leader instance per socket path.
///
/// # Arguments
///
...
/// `kigi_update::auto_update::resolve_restart_exe`.
///
...
fn spawn_leader_subprocess() -> Result<u32, ConnectionError> {
    ...
    let leader_log = std::env::var("KIGI_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "kigi_shell=info,kigi_acp_lib=warn,kigi_mcp=warn".into());
