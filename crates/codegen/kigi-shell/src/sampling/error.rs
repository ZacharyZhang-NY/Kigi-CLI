    ...
    match err {
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                kigi_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: kigi_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != kigi_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == kigi_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        unsafe {
            ...
            if let Some(v) = prev_legacy {
                std::env::set_var("KIGI_CODE_XAI_API_KEY", v);
            }
        }
        if let Err(e) = result {
