...
// Settings helpers — typed disk-write wrappers for each setting. All route
// through `update_config` → `merge_section` → `save_config`.

pub async fn set_compact_mode(value: bool) -> Result<()> {
    update_config(|cfg| cfg.ui.compact_mode = value).await
}

/// `UiConfig::show_timestamps` is `Option<bool>` — pager-side `None` means
/// "use default" — so we wrap.
...
/// The nested `contextual_hints` struct stays out of `config.toml` until a
/// tip is toggled (`skip_serializing_if`).
...
/// Caller must pass the canonical theme name (`kiginight`, `tokyonight`,
/// `auto`, etc.).
pub async fn set_theme(value: String) -> Result<()> {
    update_config(|cfg| cfg.ui.theme = Some(value)).await
}

/// `auto` is rejected by the pager's `load_auto_theme_config` filter at read
/// time to prevent circular reference.
...
/// [`super::campaigns::persist_models_default`] so an explicit user pick
/// always dismisses an active campaign nudging that default. Do not persist
/// `models.default` via raw `update_config`, or a campaign would keep
...
/// Length over [`MAX_DEFAULT_MODEL_LEN`] returns `Err` (checked in
...
/// `auto` means system locale, falling back to English.
