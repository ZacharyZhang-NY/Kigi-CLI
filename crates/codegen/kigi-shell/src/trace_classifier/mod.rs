...
/// binary. Wraps a `kigi_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: kigi_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: kigi_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
/// provider command (if `KIGI_AUTH_PROVIDER_COMMAND` is set) is
...
/// the user can act on.
pub async fn resolve_api_key(explicit: Option<&str>, kigi_home: &Path) -> Result<String> {
    ...
    if let Some(key) = non_interactive_auth_key(kigi_home).await? {
        return Ok(key);
    }
    Err(anyhow!(
...
) -> Result<kigi_sampler::SamplingClient> {
    let default_home;
    let kigi_home_path: &Path = match kigi_home {
        Some(p) => p,
        None => {
            default_home = crate::util::kigi_home::kigi_home();
            default_home.as_path()
        }
    };
    let resolved = resolve_api_key(api_key, kigi_home_path).await?;
    let config = kigi_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..kigi_sampler::SamplerConfig::default()
    };
    kigi_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    ...
    fn write_empty_auth_json(kigi_home: &Path) {
        std::fs::write(kigi_home.join("auth.json"), "{}").expect("write auth.json");
    }
    ...
    async fn resolve_api_key_precedence() {
        ...
        let kigi_home = tmp.path();
        // No auth.json at all → AuthManager sees nothing on disk.
        write_empty_auth_json(kigi_home);
        ...
        assert!(
            resolve_api_key(None, kigi_home).await.is_err(),
            "no flag, no env, no auth.json → error",
        );
        assert!(
            resolve_api_key(Some(""), kigi_home).await.is_err(),
            "empty flag → error",
        );
        assert!(
            resolve_api_key(Some("   "), kigi_home).await.is_err(),
        ...
        assert_eq!(
            resolve_api_key(Some("from-flag"), kigi_home)
                .await
                .expect("ok"),
            "from-flag",
        );
        ...
        assert_eq!(
            resolve_api_key(None, kigi_home).await.expect("ok"),
            "from-env"
        );
        assert_eq!(
            resolve_api_key(Some("from-flag"), kigi_home)
        ...
        assert_eq!(
            resolve_api_key(Some(""), kigi_home).await.expect("ok"),
            "from-env",
    ...
    async fn resolve_api_key_falls_back_to_auth_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let kigi_home = tmp.path();
        write_auth_json(kigi_home, "from-auth-json");
        // `with_no_env` returns sync; we re-implement the clearing
        // inline so we can `.await` the resolver.
        let key =
            with_env_isolated(async { resolve_api_key(None, kigi_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_skips_auth_json_when_env_provided() {
        ...
        let key = with_env_isolated(async {
            unsafe { std::env::set_var("XAI_API_KEY", "from-env") };
            let resolved = resolve_api_key(None, kigi_home).await.expect("ok");
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        let msg = with_env_isolated(async {
            let err = resolve_api_key(None, kigi_home)
                .await
    ...
    async fn resolve_api_key_skips_empty_auth_json_entry() {
        ...
        // the unified error.
        write_auth_json(kigi_home, "");
        let err1 = with_env_isolated(async { resolve_api_key(None, kigi_home).await }).await;
        assert!(err1.is_err(), "empty auth.json entry is treated as absent");

        write_auth_json(kigi_home, "   ");
        let err2 = with_env_isolated(async { resolve_api_key(None, kigi_home).await }).await;
        assert!(
            err2.is_err(),
    ...
    async fn resolve_api_key_returns_refreshable_oidc_token() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let kigi_home = tmp.path();
        write_fresh_oidc_auth_json(kigi_home, "fresh-oidc-key");
        let key =
            with_env_isolated(async { resolve_api_key(None, kigi_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_returns_error_when_oidc_token_expired_and_no_refresh_token() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let kigi_home = tmp.path();
        write_expired_oidc_auth_json_no_refresh(kigi_home, "stale-oidc-key");
        let err = with_env_isolated(async { resolve_api_key(None, kigi_home).await }).await;
        assert!(
            err.is_err(),
    ...
    /// * `KIGI_AUTH` — inline-JSON credentials override that bypasses
    ///   the on-disk read entirely (`AuthManager::new`).
    /// * `KIGI_AUTH_PATH` — overrides the auth.json path; if set to
    ///   the operator's real `~/.kigi/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `KIGI_AUTH_PROVIDER_COMMAND` — selects an external
    ...
    async fn run_rejects_missing_trace_path() {
        ...
        let args = RunArgs {
            ...
            min_confidence: None,
            include_reasoning: None,
            kigi_home: Some(tmp.path().to_path_buf()),
        };
