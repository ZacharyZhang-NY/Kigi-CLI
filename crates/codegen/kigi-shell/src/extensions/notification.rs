...
impl PromptUsage {
    ...
    pub fn project_from_ledger(
        ledger: Option<&kigi_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
...
impl From<&kigi_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &kigi_chat_state::UsageTotals) -> Self {
        ...
        let kigi_chat_state::UsageTotals {
            input_tokens,
            output_tokens,
            cached_read_tokens,
            reasoning_tokens,
...
impl From<&kigi_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &kigi_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<kigi_hooks_plugins_types::HookInfo>,
...
pub struct CompactionSegmentFile {
    pub items: Vec<kigi_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: kigi_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<kigi_chat_state::compaction_utils::CompactionAttempt>,
}
...
mod tests {
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use kigi_chat_state::compaction_utils::CompactionAttempt;
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.kigi/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.kigi/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.kigi/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.kigi/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.kigi/memory/MEMORY.md".into(),
