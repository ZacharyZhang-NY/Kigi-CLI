...
use kigi_tty_utils::KILL_REAP_TIMEOUT;
...
impl PipeReader {
    ...
    async fn join_bounded(mut self) -> Vec<u8> {
        ...
        {
            self.task.abort();
            // Await the cancellation: abort() is asynchronous, and a reader
            // past its await point could otherwise append between the take
            // below and its own completion, losing those bytes.
...
impl AsyncTerminalRunner for LocalTerminalRunner {
    async fn run(&self, request: TerminalRunRequest) -> Result<TerminalRunResult, TerminalError> {
        ...
        // output pipes open past the kill). Group teardown is an enhancement
        // over the direct-kill floor, so both enrollment failures degrade
        // softly instead of dropping an already-running shell.
        let process_group = match kigi_tty_utils::ProcessGroup::new() {
            Ok(mut group) => {
        ...
        // past the child's exit, and an unbounded join stalls on it.
