...
/// after we stop waiting. Mirrors the bound the `kigi-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("KIGI_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for kigi_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
...
fn spawn_shell_command(
    ...
    #[cfg(not(unix))]
    {
        let inv = kigi_config::shell::shell_command_argv(command);
        ...
        let mut cmd = CommandWrap::with_new(program, |cmd| {
            ...
            if kigi_sandbox::should_restrict_child_network() {
                ...
                unsafe {
                    ...
                    cmd.pre_exec(move || {
                        kigi_sandbox::child_net::install_child_network_filter(filter)
                    });
                }
            }
        });
