...
use kigi_tools::types::compat::{COMPAT_CELLS, CompatCell, CompatConfig};
use serde::Serialize;
...
/// `None` for native `.kigi`/`.agents` paths.
...
pub(super) fn resolve_inspect_compat(
    ...
    resolve_inspect_compat_with_env(effective_config, |cell| {
        kigi_config::env_bool(cell.env_var())
    })
}
...
mod tests {
    ...
    fn inspect_compat_uses_env_config_default_precedence() {
        ...
        let report = resolve_inspect_compat_with_env(Ok(&effective_config), |cell| {
            (cell.env_var() == "KIGI_CURSOR_SKILLS_ENABLED").then_some(true)
        });
