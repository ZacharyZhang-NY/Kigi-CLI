...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("KIGI_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `KIGI_LOGIN_DEVICE_FLOW` env >
...
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("KIGI_LOGIN_DEVICE_FLOW");
...
async fn run_auth_flow_inner(
    ...
    // --device-auth / KIGI_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
...
pub async fn try_ensure_fresh_auth(grok_com_config: &GrokComConfig) -> Option<GrokAuth> {
    ...
    let auth_manager = std::sync::Arc::new(AuthManager::new(&kigi_home, grok_com_config.clone()));
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    let kigi_home = kigi_home::kigi_home();
    let auth_manager = Arc::new(AuthManager::new(&kigi_home, grok_com_config.clone()));
...
/// otherwise `KIGI_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    let authenticated = if devbox {
        ...
        let kigi_home = kigi_home::kigi_home();
        let auth_manager = Arc::new(AuthManager::new(&kigi_home, config.grok_com_config.clone()));
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `KIGI_CODE_XAI_API_KEY` env var is set.
...
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let kigi_home = kigi_home::kigi_home();
    let auth_manager = AuthManager::new(&kigi_home, config.grok_com_config.clone());
...
mod tests {
    ...
    /// Run `f` with `KIGI_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("KIGI_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("KIGI_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("KIGI_LOGIN_DEVICE_FLOW"),
        };
        f()
    }
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only kigi_home: reading a missing auth.json succeeds (empty), but
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("KIGI_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("KIGI_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        // KIGI_LOGIN_DEVICE_FLOW / the real config file (the CLI override
