...
async fn reactive_401_recovery_produces_fresh_token_end_to_end() {
    ...
    assert!(mgr.try_recover_unauthorized().await);
...
/// typed transient (retryable on wake), never forces a manual re-login (a
/// lid close must never count as a forced re-login), and records
...
#[tokio::test]
async fn sleep_deferred_refresh_is_transient_no_reauth_no_verdict() {
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), GrokComConfig::default()));
    ...
    for _ in 0..4 {
        ...
        assert!(
            ...
            "a lid-close deferral must never force a manual re-login",
        );
    }
    assert!(
        mgr.permanent_failure().is_none(),
    ...
    // End-to-end through 401 recovery: a deferred recovery terminates with
    // the transient error.
...
// ── manual re-auth classification ────────────────────────────

/// Truth table for `forces_manual_reauth`: only terminal failures that force
/// the user back to `/login` count; self-healing (TTL) reasons, transient /
/// no-credential errors, and API-key lockouts don't.
#[test]
fn forces_manual_reauth_maps_terminal_and_skips_non_forcing() {
    use crate::auth::error::RefreshTokenFailedReason as Reason;
    use crate::auth::recovery::forces_manual_reauth;

    let permanent = |reason: Reason| forces_manual_reauth(&AuthError::permanent(reason));
    // A revoked refresh token forces a re-login -> counts.
    assert!(permanent(Reason::RefreshTokenRejected));
    // Every terminal pipeline error forces a re-login.
    assert!(forces_manual_reauth(&AuthError::ServerRejectedNoRecovery));
    assert!(forces_manual_reauth(&AuthError::RecoveryExhausted));
    assert!(forces_manual_reauth(&AuthError::TokenExpiredNoRefresh));
    assert!(forces_manual_reauth(&AuthError::PinnedTeamMismatch {
        message: String::new()
    }));
    // Self-healing (TTL) reasons, transient / no-credential, and API-key
    // lockouts don't count.
    assert!(!permanent(Reason::ClientRejected));
    assert!(!permanent(Reason::Other));
    assert!(!forces_manual_reauth(&AuthError::transient("x")));
    assert!(!forces_manual_reauth(&AuthError::NotLoggedIn));
    assert!(!forces_manual_reauth(&AuthError::ApiKeyAuthDisabled));
}

/// Truth table for `relay_should_cancel`: the relay gives up on any terminal
/// auth failure — including `ApiKeyAuthDisabled`, which deliberately doesn't
/// force a manual re-login — and keeps reconnecting through transient
...
fn relay_should_cancel_gives_up_only_on_terminal_failures() {
    ...
    // Cancelled even though it never forces a re-login (a kill-switched API
    // key means rotate the key, not `/login`).
