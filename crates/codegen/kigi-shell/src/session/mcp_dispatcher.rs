...
pub enum McpServerStatus {
    /// Client is in [`kigi_mcp::servers::ClientStateKind::Ready`]
...
/// [`kigi_mcp::servers::McpClientEventKind`]). Used as the
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        use kigi_mcp::servers::{McpClient, McpState};
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = kigi_acp_lib::AcpAgentGatewaySender::new(gw_tx);
