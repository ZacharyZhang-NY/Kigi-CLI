...
// kigi-interjection-core crate so the server-side agent loop can adopt the
...
pub(crate) use kigi_interjection_core::{InterjectionBuffer, drain_formatted, format_interjection};

/// Shell instantiation of the shared entry type: images are ACP content.
pub(crate) type PendingInterjection =
    kigi_interjection_core::PendingInterjection<acp::ImageContent>;
...
impl SessionActor {
    ...
    /// Inject a synthetic user message: persist to `updates.jsonl`, optionally
    /// notify the pager, and push it into the conversation context.
    ///
    /// Shared by `add_followup_message_as_user_turn` (which notifies) and
    /// `drain_pending_interjections` (which skips notification because the
    /// pager already has a local user prompt block).
