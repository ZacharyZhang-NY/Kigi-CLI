...
fn partition_rules_by_scope(
    ...
    Vec<kigi_agent::prompt::user_message::RuleEntry>,
    Vec<kigi_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.kigi/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = kigi_agent::prompt::user_message::RuleEntry::from(f);
...
impl SessionActor {
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: kigi_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use kigi_agent::prompt::agents_md::read_agents_config_with_paths;
        use kigi_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use kigi_workspace::file_system::{git_status_short, jj_status};
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &kigi_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
