...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<kigi_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<kigi_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
                   ...
                   } else { tracing::debug!(target : kigi_log::memory_log::TARGET,
                   ...
                   kigi_log::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
                   ...
                   Some(kigi_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
                   ...
                   Some(kigi_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
                   reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
                   body_bytes_after, }) => {
                   kigi_log::unified_log::info("shell.image_budget", Some(session
                   ...
                   })),); } Some(kigi_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
                   Some(kigi_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
                   ...
                   .fire_hook(kigi_hooks::event::HookEventName::SessionEnd, None,
                   kigi_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
                   ...
                   kigi_hooks::dispatcher::dispatch_non_blocking(& registry,
                   kigi_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
                   session.send_hook_execution("session_end", None, None, & results). await; }
                   let envelope = session.fire_hook(kigi_hooks::event::HookEventName::Stop,
                   None, kigi_hooks::event::HookPayload::Stop { reason : "channel_closed"
                   .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
                   .clone() { let ctx = session.hook_run_ctx(); let results =
                   kigi_hooks::dispatcher::dispatch_non_blocking(& registry,
                   kigi_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
                   ...
                   kigi_log::memory_log::TARGET, result = ? result, tool_searches =
                   ...
                   json_schema, send_now, respond_to, persist_ack } => {
                   ...
                   screen_mode, verbatim, json_schema, send_now, respond_to, persist_ack). await; if cancel_for_send_now { session
                   ...
                   .send(kigi_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
                   ...
                   kigi_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
                   ...
                   .send(kigi_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
                   ...
                   .emit(kigi_file_utils::events::Event::McpServerToggled { server_name :
                   ...
                   session.fire_hook(kigi_hooks::event::HookEventName::SessionStart, None,
                   kigi_hooks::event::HookPayload::SessionStart { source, model_id : None,
                   agent_type : None, },); if let Some(registry) = session.hook_registry
                   .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
                   kigi_hooks::dispatcher::dispatch_non_blocking(& registry,
                   kigi_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
                   ...
                   queue_meta : None, send_now : false, }); }
                   SessionActor::maybe_start_running_task(session.clone(), completion_tx
                   ...
                   respond_to.send(result); } SessionCommand::TakeStreamingCapture { prompt_id, respond_to } => { let taken
                   ...
                   .fire_hook(kigi_hooks::event::HookEventName::SessionEnd, None,
                   kigi_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
                   .to_string(), turn_count : None, tool_call_count : None, },); if let
                   Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
                   .hook_run_ctx(); let results =
                   kigi_hooks::dispatcher::dispatch_non_blocking(& registry,
                   ...
                   session.send_hook_execution("session_end", None, None, & results). await; }
                   let envelope = session.fire_hook(kigi_hooks::event::HookEventName::Stop,
                   None, kigi_hooks::event::HookPayload::Stop { reason : "shutdown"
                   .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
                   .clone() { let ctx = session.hook_run_ctx(); let results =
                   kigi_hooks::dispatcher::dispatch_non_blocking(& registry,
                   kigi_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[kigi_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = kigi_chat_state::compaction_utils::extract_user_query(&raw);
