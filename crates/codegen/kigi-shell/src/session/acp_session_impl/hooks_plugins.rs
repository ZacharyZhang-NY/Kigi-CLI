...
impl SessionActor {
    ...
    pub(super) async fn reseed_mcp_output_cap(&self) {
        ...
        let existing = resources
            .get::<kigi_tools::types::resources::TruncationCfg>()
            .map(|c| c.0.clone());
        match (resolved, existing) {
            (resolved, Some(mut cfg)) => {
                if cfg.mcp_max_output_bytes != resolved {
                    cfg.mcp_max_output_bytes = resolved;
                    resources.insert(kigi_tools::types::resources::TruncationCfg(cfg));
                }
            }
            (Some(v), None) => {
                resources.insert(kigi_tools::types::resources::TruncationCfg(
    ...
    pub(super) async fn handle_hooks_action(
        ...
        use kigi_hooks_plugins_types::{ActionOutcome, HooksAction, OutcomeStatus};
        ...
        match action {
            ...
            HooksAction::Add { path } => {
                ...
                // CWE-427: add_hooks_path() validates path is under ~/.kigi/.
            ...
            HooksAction::Disable { hook_name } => {
                match kigi_hooks::trust::disable_hook(&hook_name) {
                    Ok(()) => ActionOutcome {
                        status: OutcomeStatus::Success,
            ...
            HooksAction::Enable { hook_name } => match kigi_hooks::trust::enable_hook(&hook_name) {
            ...
            HooksAction::ToggleSource {
                ...
                for name in &hook_names {
                    let ok = if disable {
                        kigi_hooks::trust::disable_hook(name).is_ok()
                    } else {
                        kigi_hooks::trust::enable_hook(name).is_ok()
                    };
                    if ok {
                        toggled += 1;
                    }
    ...
    pub(super) async fn handle_plugins_action(
        ...
        use kigi_hooks_plugins_types::{ActionOutcome, OutcomeStatus, PluginsAction};
        ...
        match action {
            ...
            PluginsAction::Install { source } => {
                ...
                match kigi_agent::plugins::git_install::install_from_source(
                    &install_source,
                    &registry,
                ) {
                    Ok(result) => {
                        let repo = kigi_agent::plugins::git_install::build_installed_repo(
                            &result,
                            &install_source,
                        );
            ...
            PluginsAction::Uninstall {
                ...
                let mut registry = kigi_agent::plugins::InstallRegistry::load();
            ...
            PluginsAction::Update { plugin_id } => {
                ...
                let repos_to_update: Vec<(
                    String,
                    kigi_agent::plugins::install_registry::InstalledRepo,
                ...
                for (key, repo) in &repos_to_update {
                    match kigi_agent::plugins::git_install::update_repo(key, repo) {
                        Ok(status) => {
                            use kigi_agent::plugins::git_install::UpdateStatus;
                            match status {
    ...
    pub(super) async fn reload_hooks_impl(self: &std::sync::Arc<Self>) -> String {
        let git_root = kigi_workspace::session::git::find_git_root_from_path(std::path::Path::new(
    ...
    pub(super) async fn reload_plugins_impl(
        self: &Arc<Self>,
        handle: &kigi_agent::plugins::SharedPluginRegistryHandle,
        force: bool,
    ) -> String {
        let session_cwd = std::path::Path::new(&self.session_info.cwd);

        let sid = self.session_info.id.0.as_ref();
        kigi_log::unified_log::info("reload_plugins_impl: start", Some(sid), None);
    ...
    pub(crate) fn preserve_session_plugin_dirs(
        &self,
        incoming: Option<std::sync::Arc<kigi_agent::plugins::PluginRegistry>>,
    ...
    pub(super) async fn apply_plugin_registry_snapshot(
        self: &Arc<Self>,
        new_registry_snapshot: Option<std::sync::Arc<kigi_agent::plugins::PluginRegistry>>,
    ) -> (usize, bool, usize) {
        let sid = self.session_info.id.0.as_ref();
        let session_cwd = std::path::Path::new(&self.session_info.cwd);
        ...
        *self.plugin_registry.borrow_mut() = new_registry_snapshot.clone();
        ...
        if let Some(ref new_registry) = new_registry_snapshot {
            ...
            hooks_reloaded = new_specs.len();
            {
                ...
                if let Some(ref mut arc_reg) = *reg {
                    ...
                    let (mut new_reg, _) = kigi_hooks::discovery::load_hooks_from_sources(&[], &[]);
        ...
        let mcp_changed = if let Some(diff) = mcp_diff {
            ...
            {
                let _ = tx.send(kigi_mcp::servers::McpClientEvent::ConfigDiff {
