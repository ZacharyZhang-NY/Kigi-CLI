...
impl SessionActor {
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            kigi_workspace::session::file_state::FlexiblePath,
            Option<String>,
    ...
    pub(super) async fn handle_repair_history(
        &self,
        dry_run: bool,
    ) -> anyhow::Result<kigi_chat_state::compaction_utils::HistoryRepairReport> {
        ...
        if turn_flag.load(std::sync::atomic::Ordering::SeqCst) {
            anyhow::bail!(kigi_chat_state::commands::RepairHistoryBlocked);
        }
