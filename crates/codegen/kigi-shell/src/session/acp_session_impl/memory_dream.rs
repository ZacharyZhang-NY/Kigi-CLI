...
pub(super) struct MemoryFlushSnapshot {
    counts: kigi_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &kigi_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                kigi_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                kigi_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: kigi_log::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        tracing::info!(target: kigi_log::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
    ...
    pub(super) async fn snapshot_memory_flush_state(&self) -> MemoryFlushSnapshot {
        ...
        let chat_history = crate::sampling::conversation_to_chat_messages(
            kigi_chat_state::compaction_utils::prepare_conversation_for_summarization(conversation),
        );
        MemoryFlushSnapshot {
            counts,
            chat_history,
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    kigi_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = kigi_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                kigi_sampler::collect_response(events).await
            }
        };
