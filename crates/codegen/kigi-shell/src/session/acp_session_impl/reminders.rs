...
pub(crate) fn resolve_reminder_policy(
    ...
    todo_gate: bool,
) -> kigi_agent::ReminderPolicy {
    let mut policy = kigi_agent::ReminderPolicy::default();
...
pub(super) fn todo_gate_active(
    policy: &kigi_agent::system_reminder::ReminderPolicy,
    audience: kigi_agent::prompt::context::PromptAudience,
...
impl SessionActor {
    ...
    pub(super) async fn drain_between_turn_completions(&self) {
        ...
        if !bash_completions.is_empty() {
            ...
            if goal_loop_active {
                ...
                let task_output_name =
                    kigi_tools::reminders::task_completion::resolve_task_output_tool_name(&bridge)
                        .await;
                let read_tool_name =
                    kigi_tools::reminders::task_completion::resolve_read_tool_name(&bridge).await;
                let reminder =
                    kigi_tools::reminders::task_completion::format_between_turn_bash_completions(
                        &bash_completions,
        ...
        let reminder =
            kigi_tools::reminders::task_completion::format_between_turn_completion_reminder(
                &completions,
                &bridge,
            )
            .await;
    ...
    pub(super) fn todo_gate_policy(&self) -> Option<kigi_agent::system_reminder::TodoGateConfig> {
    ...
    pub(super) async fn collect_todo_gate_input(&self, prompt_id: &str) -> CollectedTodoGateInput {
        ...
        let incomplete_terminal_tasks = bridge
            ...
            .await
            .into_iter()
            .filter(kigi_tools::computer::types::TaskSnapshot::is_outstanding)
            .count();
