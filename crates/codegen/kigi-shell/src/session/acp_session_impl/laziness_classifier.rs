        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let kigi_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
            ...
            ConversationItem::Reasoning(r) => {
                if include_reasoning {
                    let text = kigi_sampling_types::reasoning_item_text(r);
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
    max_items: usize,
) -> Vec<kigi_workspace::permission::ClassifierTurn> {
        ...
        match item {
            ConversationItem::User(user) => {
                ...
                if !text.is_empty() {
                    ...
                    let text =
                        kigi_tools::util::truncate_str_with_marker(&text, CLASSIFIER_TURN_MAX_LEN)
                            .into_owned();
            ...
            ConversationItem::Assistant(assistant) => {
                for tc in &assistant.tool_calls {
                    ...
                    let args =
                        kigi_tools::util::truncate_str_with_marker(&args, CLASSIFIER_TURN_MAX_LEN)
                            .into_owned();
