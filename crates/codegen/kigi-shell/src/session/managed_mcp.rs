...
impl McpDisabledReason {
    ...
    pub fn for_blocked_server(
        policy: &kigi_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &kigi_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: kigi_tools::util::kigi_home::kigi_home().join("config.toml"),
    };
...
fn load_plugin_mcp_servers_from_value(
    ...
    let normalized = kigi_agent::plugins::manifest::normalize_inline_mcp_servers(root);
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = kigi_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
mod tests {
    ...
    fn lower_precedence_http_servers_are_blocked_by_toml_name_claims() {
        ...
        std::fs::create_dir_all(cwd.path().join(".kigi")).unwrap();
        std::fs::write(
            cwd.path().join(".kigi").join("config.toml"),
            r#"
[mcp_servers.github]
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        let config: crate::util::config::McpConfig = serde_json::from_value(serde_json::json!({
            "mcpServers": {
                "echo-mcp": {
                    "command": "python3",
                    "args": ["${KIGI_PLUGIN_ROOT}/mcp-echo-server.py"]
                }
            }
        }))
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.kigi/plugins/team-tool",
            "/home/user/.kigi/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.kigi/plugins/team-tool/mcp-echo-server.py"]
                );
            }
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use kigi_agent::plugins::PluginRegistry;
        use kigi_agent::plugins::PluginScope;
        use kigi_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use kigi_agent::plugins::manifest::{PathOrInline, PluginManifest};
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use kigi_agent::plugins::PluginScope;
        use kigi_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use kigi_agent::plugins::manifest::PluginManifest;
