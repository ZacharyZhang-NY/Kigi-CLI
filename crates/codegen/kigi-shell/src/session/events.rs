...
//! `kigi-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use kigi_file_utils::events::tracker::EventTracker;
pub(crate) use kigi_file_utils::events::types::{
...
) -> Option<kigi_sampling_types::PriorTurnInterrupt> {
    use kigi_sampling_types::PriorTurnInterrupt;
    match category {
...
/// (`kigi-file-utils`). The two enums are intentional mirrors and live in
