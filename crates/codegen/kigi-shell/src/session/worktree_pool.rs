...
//!   under `~/.kigi/worktree_pool/<instance_id>/` with a `.pid` liveness
...
use kigi_fast_worktree::{WorktreeBuilder, WorktreeSync};
...
use kigi_tty_utils::git_command;
...
/// `~/.kigi/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.kigi/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.kigi/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.kigi/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.kigi/`.
...
/// `CleanAll` mode. Placed under `~/.kigi/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(kigi_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(kigi_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                kigi_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || kigi_fast_worktree::remove_worktree(&path_owned)).await;
