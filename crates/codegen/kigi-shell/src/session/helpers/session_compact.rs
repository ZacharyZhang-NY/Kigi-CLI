...
pub(crate) use kigi_sampling_types::is_context_length_error;
...
/// input path). Tool *use* is forbidden by the prompt instruction;
/// `tool_choice` says what goes on the wire next to the attached tools
/// (ChatCompletions, Responses — some servers reject `"none"`, hence the
/// `auto` default). The Messages wire enum has no `none` either way.
///
    ...
    let output = match sampling_config.api_backend {
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = kigi_sampling_types::StopReason::from(fr);
                                truncated = matches!(sr, kigi_sampling_types::StopReason::Length);
        ...
        ApiBackend::Responses => {
            let request = ConversationRequest {
                items: chat_history,
                // Hosted tools count: a hosted-only request with the choice
                // omitted would leave the hosted tool callable mid-compact.
                tool_choice: (!tools.is_empty() || !hosted_tools.is_empty())
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            kigi_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            kigi_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: kigi_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            kigi_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                        sr, kigi_sampling_types::messages::StopReason::MaxTokens
                                        |
                                        kigi_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                    );
                                    stop_reason = Some(
                                        match sr {
                                            kigi_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            kigi_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            kigi_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            kigi_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            kigi_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            kigi_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            kigi_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            kigi_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );
...
mod compacted_history_shape_tests {
    ...
    /// `kigi-chat-state`, rendering the system-reminder synchronously (no
    ...
    async fn test_compacted_history_raw_strings() {
        ...
        let formatted_summary =
            kigi_chat_state::compaction_utils::format_compact_summary_content(compaction_summary);
        assert_eq!(
    ...
    fn fallback_minimal_history_has_no_tool_results() {
        use kigi_chat_state::compaction_utils::validate_compacted_history;
...
mod reasoning_compaction_regression_tests {
    ...
    /// (prompt-prefix/KV-cache alignment) with the configured `tool_choice`
    /// (`auto` default; `none` when configured), and must omit both keys when
    /// no tools are passed (Chat Completions rejects a bare `tool_choice`).
    #[tokio::test]
    async fn chat_completions_compaction_attaches_tools_with_configured_tool_choice() {
        ...
        .await
        .unwrap_or_else(|_| panic!("compaction with tool_choice none must succeed"));
        let bodies = captured.lock().unwrap();
        assert_eq!(bodies.len(), 3, "mock must have served all three requests");
        let with_tools = &bodies[0];
        assert_eq!(
            with_tools["tool_choice"],
            json!("auto"),
            "default tool_choice is auto — some backends reject none; the prompt forbids tool use"
        );
        ...
        let none_configured = &bodies[2];
        assert_eq!(
            none_configured["tool_choice"],
            json!("none"),
            "configured none must reach the wire"
        );
        let _ = shutdown_tx.send(());
    }
    #[tokio::test]
