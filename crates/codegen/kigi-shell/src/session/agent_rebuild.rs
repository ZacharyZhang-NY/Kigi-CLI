...
//! [`kigi_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`kigi_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`kigi_agent::Agent`] owns an [`kigi_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`kigi_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use kigi_agent::config::AgentDefinition;
use kigi_agent::error::AgentBuildError;
use kigi_agent::prompt::context::PromptAudience;
...
use kigi_tools::types::SharedApiKeyProvider;
use kigi_tools::types::compat::CompatConfig;
use kigi_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<kigi_tools::SharedAttributionCallback>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        agent
            .tool_bridge()
            .update_resource(kigi_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(kigi_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        {
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            kigi_tools::computer::local::LocalTerminalBackend::new_local(
                kigi_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
