...
use kigi_sampling_types::{ApiBackend, ConversationItem};
use std::sync::Arc;
...
/// Override with `KIGI_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("KIGI_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("KIGI_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        kigi_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("KIGI_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG KIGI_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(kigi_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// (`kigi-chat-state` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        self.maybe_pre_compaction_flush(total_tokens, context_window, "pre_compaction")
            .await;
        if let Err(e) = self
            .run_compact_inner(user_context, None, CompactionTrigger::Manual)
            .await
        {
            let span = tracing::Span::current();
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    kigi_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    kigi_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if kigi_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        self.dispatch_hook(
            kigi_hooks::event::HookEventName::PreCompact,
            kigi_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            kigi_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            kigi_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            kigi_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let effective_tool_defs: Vec<kigi_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            kigi_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<kigi_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(kigi_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<kigi_sampling_types::HostedTool> = if use_backend_search {
        ...
        let estimated_input_tokens =
            kigi_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, CompactionTrigger::Auto);
        ...
        let fr_config = kigi_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match kigi_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(kigi_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(kigi_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(kigi_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = kigi_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    kigi_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
        ...
        let memory_ref: Option<&dyn kigi_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn kigi_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : kigi_log::memory_log::TARGET, count = n,
        ...
        self.dispatch_hook(
            kigi_hooks::event::HookEventName::PostCompact,
            kigi_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &kigi_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = kigi_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        let result = self
            .run_compact_inner(None, None, CompactionTrigger::Auto)
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<kigi_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    fn api_error_with_context_window(context_window: u64) -> kigi_sampler::SamplingErrorInfo {
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = kigi_sampler::SamplingErrorInfo {
                    kind: kigi_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = kigi_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = kigi_chat_state::CompactionMode::Summary;
