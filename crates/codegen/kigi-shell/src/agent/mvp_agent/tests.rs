...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = kigi_agent::AgentDefinition::from_json(&serde_json::json!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        kigi_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("KIGI_AGENT", v) },
        None => unsafe { std::env::remove_var("KIGI_AGENT") },
    }
}
#[test]
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    let builder = kigi_tools::registry::types::ToolRegistryBuilder::new();
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = kigi_hunk_tracker::HunkTrackerActor::spawn(
        ...
        hunk_event_tx,
        kigi_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            kigi_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(kigi_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), kigi_chat_state::AuthType::ApiKey,);
}
...
/// Session-actor tests use `tokio::task::spawn_local`, so run the body
/// inside a current-thread `LocalSet`.
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = kigi_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
/// Regression (post-cancel slot hang): a cancel racing the prompt it targets
/// must land on the actor mailbox AFTER that prompt, or it hits an idle
/// actor as a no-op and the prompt then starts a turn nothing cancels.
/// The dispatch lock (`dispatch_locks`) provides that ordering.
#[test]
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use kigi_test_support::EnvGuard;
    use kigi_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("KIGI_SHARE_DIR", home.path());
    let _sim = EnvGuard::set(kigi_version::TEST_VERSION_ENV, "0.0-sim");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, kigi_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(kigi_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use kigi_test_support::EnvGuard;
    use kigi_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("KIGI_SHARE_DIR", home.path());
    let _sim = EnvGuard::set(kigi_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("KIGI_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(kigi_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
