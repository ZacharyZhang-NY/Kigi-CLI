...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<kigi_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub parent_terminal_backend: Option<Arc<dyn kigi_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle: Option<kigi_tools::notification::types::ToolNotificationHandle>,
    ...
    pub parent_chat_state: Option<kigi_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<kigi_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<kigi_tools::registry::types::ToolConfig>>,
    /// Parent session's agent config snapshot.
    pub agent_config: Option<crate::agent::config::Config>,
    pub hook_registry: Option<std::sync::Arc<kigi_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<kigi_workspace::permission::PermissionHandle>,
    ...
    pub workspace_ops: kigi_workspace::WorkspaceOps,
    ...
    pub parent_tool_snapshot: Option<Vec<kigi_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<kigi_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: kigi_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: kigi_tools::types::compat::CompatConfig,
    /// Shared set of IDs delivered via auto-wake synthetic prompts.
    pub auto_wake_delivered: Option<kigi_tools::reminders::task_completion::AutoWakeDeliveredIds>,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut kigi_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("KIGI_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("KIGI_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
use kigi_subagent_resolution::ResumeSourceData;
...
use kigi_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &kigi_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (kigi_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &kigi_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (kigi_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &kigi_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &kigi_sampler::SamplerConfig,
) {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(kigi_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[kigi_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<kigi_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<kigi_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        kigi_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[kigi_sampling_types::conversation::ConversationItem],
) -> bool {
    use kigi_sampling_types::conversation::ConversationItem;
    matches!(
...
fn verbatim_or_normalize_fork(
    items: Vec<kigi_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = kigi_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        kigi_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
    ...
    if let Some(source) = resume_source {
        ...
        return match storage.copy_session_data_sync(
            ...
            Ok(result) => {
                ...
                let estimated_tokens = kigi_chat_state::estimate_conversation_tokens(&conversation);
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &kigi_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use kigi_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<kigi_agent::config::AgentDefinition> {
    let mut def = kigi_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<kigi_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || kigi_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = kigi_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut kigi_agent::config::AgentDefinition,
) {
...
fn resolve_subagent_permission_mode(
    requested: kigi_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> kigi_agent::config::PermissionMode {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    kigi_workspace::session::git::find_main_repo_root_from_path(&source_cwd).unwrap_or(source_cwd)
}
...
/// auto-backgrounded (non-destructively). Env override: `KIGI_SUBAGENT_AWAIT_BUDGET_MS`.
const SUBAGENT_AWAIT_BUDGET: std::time::Duration = std::time::Duration::from_secs(600);
fn subagent_await_budget() -> std::time::Duration {
    std::env::var("KIGI_SUBAGENT_AWAIT_BUDGET_MS")
        .ok()
...
fn cancellation_error_message(
    category: Option<kigi_file_utils::events::types::CancellationCategory>,
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<kigi_tools::reminders::task_completion::AutoWakeDeliveredIds>,
    ...
    let message = kigi_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = kigi_tools::reminders::wrap_reminder(&message);
