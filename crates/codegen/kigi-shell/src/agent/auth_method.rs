...
/// `KIGI_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `grok login`, set XAI_API_KEY, or add api_key to ~/.kigi/config.toml.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.kigi/auth.json".to_string())),
    )
}
...
mod tests {
    ...
    /// `~/.kigi/config.toml` skeleton from the bug report, walks it through
    ...
    /// Legacy `KIGI_CODE_XAI_API_KEY` env var is accepted as a fallback
    ...
    /// When both `XAI_API_KEY` and `KIGI_CODE_XAI_API_KEY` are set,
    ...
    // When this token is present via the `KIGI_AUTH` env var (or via legacy
    ...
    // to-end. A regression in KIGI_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `KIGI_AUTH` env var, with no other auth
    ...
    fn grok_login_legacy_token_does_not_require_login() {
        ...
        let _g1 = EnvGuard::unset("KIGI_AUTH_PATH");
        ...
        // Provide it via KIGI_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("KIGI_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in KIGI_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("KIGI_AUTH");
        let _g2 = EnvGuard::unset("KIGI_AUTH_PATH");
