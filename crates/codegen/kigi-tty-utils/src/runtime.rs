...
//! HPC login nodes) that pins 100+ thread slots per kigi process against
//! per-user ceilings — systemd user-slice `pids.max` (commonly 1000) or
//! `RLIMIT_NPROC` — and later thread spawns die with EAGAIN. Kigi's runtimes
...
//! the workspace derives its worker count from here so the policy cannot
//! drift across crates.

use std::num::NonZeroUsize;

/// Maximum runtime worker threads for any kigi process.
