...
//! do not provide. When `$HOME` (and thus `~/.kigi`) is NFS-mounted on
...
impl JournalMode {
    ...
    /// yet), so callers must create it first. `KIGI_SQLITE_JOURNAL_MODE`
    /// (`wal`|`truncate`) overrides detection as a field kill-switch.
    pub fn for_db_path(db_path: &Path) -> Self {
        let env = std::env::var("KIGI_SQLITE_JOURNAL_MODE").ok();
        match mode_from_env(env.as_deref()) {
            EnvOverride::Mode(mode) => {
                ...
                tracing::info!(
                    ...
                    mode = mode.as_str(),
                    source = "env",
                    "sqlite journal mode forced by KIGI_SQLITE_JOURNAL_MODE"
                );
                return mode;
            }
            EnvOverride::Invalid => {
                ...
                tracing::warn!(
                    ...
                    "invalid KIGI_SQLITE_JOURNAL_MODE (accepted: wal, truncate); using detection"
                );
            }
            EnvOverride::Unset => {}
        }
    ...
    /// `set_journal_mode` in `kigi-fast-worktree`).
...
/// Parse result of the `KIGI_SQLITE_JOURNAL_MODE` kill-switch.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum EnvOverride {
    Unset,
...
/// `KIGI_SQLITE_JOURNAL_MODE` override parsing (pure for testability).
