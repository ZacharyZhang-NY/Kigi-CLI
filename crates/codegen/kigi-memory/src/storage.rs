...
//! `~/.kigi/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.
...
/// Write-operation scope. Distinct from `kigi_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.kigi/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.kigi/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.kigi/memory/`
    global_dir: PathBuf,
    /// `~/.kigi/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.kigi/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| kigi_home().join("memory"));
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        kigi_sqlite_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.kigi/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
