...
pub struct RemoteSettings {
    ...
    /// default (on). Sits below env `KIGI_FOLDER_TRUST`, user
    ...
    /// `KIGI_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// `0` disables. Env (`KIGI_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`KIGI_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`KIGI_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
