...
/// Build the unspawned proxy command. Secrets (`KIGI_SERVER_KEY` /
/// `KIGI_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT kigi_tty_utils::detach_command: the proxy must stay in
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "KIGI_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
