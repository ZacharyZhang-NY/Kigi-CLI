...
//!
//! Moved from `kigi-shell/src/session/worktree.rs` into the workspace
...
use kigi_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
pub(crate) fn to_creation_mode(t: WorktreeType) -> kigi_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => kigi_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => kigi_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => kigi_fast_worktree::CreationMode::GitCheckout,
    }
}
...
/// Returns `~/.kigi/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    kigi_home().join("worktrees").join(slug)
}
...
/// Walk up from `cwd` (staying within `~/.kigi/worktrees/`) to its registered
/// worktree record.
///
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to kigi_fast_worktree::WorkingTreeMode
    ...
    // Use kigi-fast-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(kigi_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        kigi_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        kigi_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        kigi_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        kigi_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        kigi_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.kigi/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        kigi_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use kigi-fast-worktree -- it handles copying from any worktree path
...
use kigi_fast_worktree::{
    ...
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_kigi_home,
};
...
pub fn worktree_db_rebuild() -> Result<kigi_fast_worktree::RebuildReport> {
    let home = resolve_kigi_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = kigi_fast_worktree::ListFilter {
...
/// Scan `~/.kigi/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[kigi_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    /// Point `KIGI_SHARE_DIR` at an isolated tempdir (`resolve_kigi_home` re-reads
    ...
    /// and restores `KIGI_SHARE_DIR` on drop (before releasing the lock), so the
    ...
    fn worktree_db_fixture(
        ...
        let env = LockedTestEnv::lock().set("KIGI_SHARE_DIR", &home);
