...
use kigi_hunk_tracker::{HunkId, HunkTrackerSnapshot, HunkTurnDelta};
...
impl TurnBoundary {
    ...
    /// Turn-hook end (from `on_after_turn`): activity bookkeeping (+ optional
    /// rewind finalize under `workspace_rewind_all_outcomes`).
    pub(crate) fn turn_end(turn_number: u64, duration_ms: u64, outcome: TurnHookOutcome) -> Self {
        Self::End {
            prompt_index: None,
            turn_number,
            duration_ms,
...
/// Resolve `workspace_rewind_hunks` from `KIGI_WORKSPACE_REWIND_HUNKS` (default off).
pub(crate) fn rewind_hunks_enabled() -> bool {
    kigi_config::env_bool("KIGI_WORKSPACE_REWIND_HUNKS").unwrap_or(false)
}
/// Resolve `workspace_rewind_durable` from `KIGI_WORKSPACE_REWIND_DURABLE`
/// (default off). Off ⇒ the legacy in-memory-only path with no disk I/O.
pub(crate) fn rewind_durable_enabled() -> bool {
    kigi_config::env_bool("KIGI_WORKSPACE_REWIND_DURABLE").unwrap_or(false)
}
impl WorkspaceSession {
    ...
    pub(crate) async fn restore_hunk_checkpoints(&self, target_prompt_index: usize) {
        ...
        let mut file_states: HashMap<std::path::PathBuf, kigi_hunk_tracker::FileHunkStateSnapshot> =
            HashMap::new();
