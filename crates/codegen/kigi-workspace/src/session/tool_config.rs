...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<kigi_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<kigi_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn kigi_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            kigi_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = kigi_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// `KIGI_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
mod tests {
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use kigi_tools::types::resources::{State, WebCitationCounter};
