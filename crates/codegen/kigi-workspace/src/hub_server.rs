...
use kigi_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
use kigi_tools::implementations::grok_build::scheduler::interval::interval_to_human;
use kigi_tools::implementations::grok_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use kigi_tools::registry::types::FinalizedToolset;
use kigi_tools::types::resources::Terminal;
...
) -> Vec<kigi_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use kigi_tools::computer::types::TaskKind;
    use kigi_tools::types::resources::Terminal;
    use kigi_tools::types::tool::ToolKind;
    use kigi_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &kigi_tools::registry::types::FinalizedToolset,
) -> Vec<kigi_workspace_types::rpc::workspace::TodoSummaryWire> {
    use kigi_tools::implementations::grok_build::todo::{TodoState, TodoStatus};
    use kigi_tools::types::resources::State;
    use kigi_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use kigi_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use kigi_workspace_types::rpc::search::FuzzyStatusReq;
        use kigi_workspace_types::rpc::skills::DiscoverPluginsReq;
        use kigi_workspace_types::rpc::workspace::{
        ...
        use kigi_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<kigi_tool_runtime::SessionContext>();
    ...
    async fn handle_hook(&self, session_id: SessionId, frame: HookFrame) {
        match frame.event {
            ...
            HookEvent::Custom { kind, payload } => {
                use kigi_tool_protocol::turn_hook::{
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use kigi_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    use kigi_tool_protocol::turn_hook;
    use kigi_tools::implementations::grok_build::scheduler::types::ScheduledTask;
    /// Helper: consume the first item from a ToolStream.
    async fn next_item(
        stream: &mut ToolStream<TypedToolOutput>,
    ) -> Option<kigi_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use kigi_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use kigi_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                "GrokBuild:read_file",
                Some(kigi_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(kigi_tool_runtime::SessionContext("main".to_owned()));
