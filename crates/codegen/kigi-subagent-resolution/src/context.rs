...
//!
//! Extracted from `kigi-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `kigi-chat-state::compaction_utils::strip_system_tags` which
...
/// `kigi-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use kigi_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(kigi_sampling_types::synthesized_reasoning_item(text))
    }
